...
csvTab <- xtable(metadata[order(metadata$Species),], sanitize.text.function=function(x){x}, label='tbl:replicator-species', caption='All observed multipliers labelled by single character or digit species identifier. Each multiplier is represented as a series of reactions, numbered in sequence.',
)
