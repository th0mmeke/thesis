...
csvTab <- xtable(metadata[order(metadata$Species),], sanitize.text.function=function(x){x}, label='tbl:replicator-species', caption='All reaction cycles in observed multipliers labelled by single character or digit species identifier. Each cycle is represented as a series of reactions, numbered in sequence.',
)
