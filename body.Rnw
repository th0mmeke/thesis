...
\section{Relationships with other fields}\label{relationships-with-other-fields}
...
\subsection{Biological Evolution}\label{evolution}
...
\subsection{Evolution without life}\label{evolution-without-life}
...
Seems tightly linked to life--requirements for OEE (hypothesised) look a lot like requirements for Life (which is only known OEE system, so perhaps not so surprising)
