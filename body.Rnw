...
% HIGHSTART
% 2860d6fe-hs - ecf 0,1,5,10 <- MAIN DATASET FOR HIGHSTART
% d75bc08cf - High start case with four-level factors for p_reproduction and p_selection, ecf=1,5,10 
% 6852f455 - High start = True, P_REPRODUCE+P_SELECTION changes, X3 only 1 (fitness-based sampling)
% 81c7c59 - High start = True, 
% d0fa13939 - High start = True

% LOWSTART
% 2860d6fe - Low-start with correct labels, ecf=0,1,5,10 <- MAIN DATASET FOR LOWSTART
% 1203f34d3 - Low-start case with four-level factors for p_reproduction and p_selection, ecf=0
% c45e9bc87 - Low-start case with four-level factors for p_reproduction and p_selection, ecf=1,5,10
% dbb85db63.rds - see derivation below - from commit dbb85db63
% Datasets with wrong sort order for implementation of Truncation function:
% 041b62df - 4915024467 with P_SELECTION changes (to 0.33, 0.66) (but ecf only 0=fixed)
% e22448db3 - 4915024467 with P_REPRODUCE changes (to 0.33, 0.66)
% e124ce423 - 4915024467 with P_REPRODUCE+P_SELECTION changes (both to 0.33,0.66)
% d9f18e2f5e  Low-start, no truncation code/only runs are with factor[3] = 0
% 4915024467 - Low-start
...
\section{TODO-Approach}\label{approach}
...
\subsection{TODO-Start with evolution}\label{start-with-evolution}
...
\subsection{TODO-Preliminary Investigation}\label{preliminary-investigation}
...
\subsection{TODO-Evolutionary Potential}\label{evolutionary-potential}

\subsection{TODO-Search for Creativity}\label{search-for-creativity}
...
\chapter{TODO-Model and initial factor screening}
...
All experiments in this part of the work make use of the same base model (\autoref{base-model-algorithm}) where the key elements of the hypothesis, such as inheritance fidelity, are made explicit parameters. The main elements will be quite familiar to anyone from Evolutionary Computation or Evolutionary Biology, although the introduction of fidelity is novel and important to the overall thesis.
...
\begin{itemize}
	...
	\item \emph{Fidelity} measures the correlation between the child's value for a property and the same trait's value in the parent. The range is $[0,1]$ where a value of $0$ means that the value for a child's property has no correlation with its parent's value for that property. High \emph{fidelity} values mean high correlation, and when $fidelity = 1.0$ the child's value is identical to the parent's. \TODO{fidelity vs inheritance - subtle difference? Inheritance is the goal, fidelity is the degree of correlation between two generations, also the mechanism by which variation is introduced\ldots}
\end{itemize}
...
\begin{table}
	\begin{center}
		...
		\begin{tabular}{@{}llp{8cm}@{}}
			...
			Correlate Fitness&        	$\{\mathrm{true}, \mathrm{false}\}$&  If true, child's fitness is related to parent's fitness by parent's fidelity, and if false, by a random value $x\in \mathcal{U}[0,1]$\\
...
In this case, entities start with a higher initial fidelity and a greater range of fitnesses (see \cref{tbl:ic}). This case examines the ability of the model to preserve advantageous entities, although, in the broader scope of the thesis, it is highly unlikely that any randomly created population would be of this initial form.
...
\section[Upper size bounds]{Investigation of effect of upper bound on population size}\label{check-effect-of-truncation}
...
\subsection[Experiment 1]{Effect of population limits (experiment \#1)}

% EXPERIMENT 1 - UNBOUNDED POPULATIONS
% Dataset: df_d9f18e2

<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df_d9f18e2 <- load_df('results/results-d9f18e2.data') # unbounded dataset
df_d9f18e2_fixed <- df_d9f18e2[df_d9f18e2$environment_change_frequency==0,]
@

The environment is considered ``fixed'' (that is, each element's fitness value does not change); the initial population consists of \Sexpr{df_d9f18e2[1,]['pop']} elements constructed according to the Low-start case (\cref{Low-start-case}) and the maximum number of generations equals \Sexpr{max(df_d9f18e2['gen'])} generations.

<<unboundedplot, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='On the left, a histogram of number of generations achieved by an unbounded algorithm before either i) the population size increases beyond 10x the original population size, or ii) the simulation succeeds in reaching the expected number of generations (which never occurred in an unbounded population). On the right, the growth of population size grouped by replicate (that is, by common factor levels) showing rapid population growth before reaching the 10x practicality limit.'>>=
ap <- ggplot(aggregate(df_d9f18e2_fixed$gen,Filter(is.factor,df_d9f18e2_fixed),max), aes(x))+geom_histogram(binwidth=1,origin = .5) + labs(x="Final generation", y="Count of Runs") + scale_x_continuous(breaks=1:12) + scale_y_continuous(limits=c(0,2),breaks=c(0,1,2))
bp <- ggplot(df_d9f18e2_fixed,aes(x=gen,y=pop,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,truncate,distribution))) + geom_line() + geom_point() + scale_x_continuous(breaks=1:100) + labs(x="Generation", y="Population size")
...
\subsection[Experiment 2]{Is the choice of limit mechanism significant? (experiment \#2)}

% EXPERIMENT 2 - CHOICE OF LIMIT MECHANISM
% Dataset: dbb85db63

<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df_low <- load_df('results/results-2860d6fe.data')
df_completed <- subset(df_low, gen==500)
@

Given then that an upper bound is needed for practicality, this section describes two possible approaches and attempts to understand the form of the bias each introduces into the experimental results. 
...
Data is from a Low-start dataset, made up of \Sexpr{nrow(subset(df_low,gen==0))} runs under fixed conditions, of which we are exclusively concerned with the subset of \Sexpr{nrow(df_completed)} runs that reached completion at \Sexpr{max(df_completed['gen'])} generations.

The runs that reached completion are approximately evenly distributed between the two methods, fitness-independent (\Sexpr{nrow(df_completed[df_completed['truncate']==-1,])} runs) and fitness-based (\Sexpr{nrow(df_completed[df_completed['truncate']==1,])}), indicating that the two methods at least appear comparable with respect to their affect on population size.
...
<<limitcomparisonplots, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Summary plots for fitness-independent sampling ("Independent") and fitness-based or dependent selection ("Dependent"), where the top line contains density plots and the bottom line boxplots showing ranges for both final average fidelity (on the left-hand side) and final average fitness (on the right).'>>=
 df_completed$temp <- factor(df_completed$truncate, labels=c("Independent","Dependent"))
 ap <- qplot(ave_cor, facets=temp ~ ., geom="density", data=df_completed, xlab="Fidelity", ylab="Density")
 bp <- qplot(ave_fit, facets=temp ~ ., geom="density", data=df_completed, xlab="Fitness", ylab="Density")
 cp <- qplot(temp, ave_cor, geom="boxplot", data=df_completed, xlab="",ylab="Fidelity")
 dp <- qplot(temp, ave_fit, geom="boxplot", data=df_completed, xlab="",ylab="Fitness")
 grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
@

<<limitqqplots, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='\\Gls{qq} plots comparing fitness-independent and fitness-based or dependent results for fidelity (left) and fitness (right).'>>=
 df_independent <- subset(df_completed,temp=="Independent")
 df_dependent <- subset(df_completed,temp=="Dependent")
 ap <- myqqplot(sort(df_independent$ave_cor), sort(df_dependent$ave_cor), main="Final average fidelity", xlab="Independent",ylab="Dependent")
 bp <- myqqplot(sort(df_independent$ave_fit), sort(df_dependent$ave_fit), main="Final average fitness", xlab="Independent",ylab="Dependent")
...
<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
temp <- load_df('results/results-2860d6fe.data')
...
<<lowstartfactorinfluence, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Influence of factors'>>=
m0 <- lm(ave_cor~(p_reproduce+p_selection+n_offspring+distribution+fitness_correlation)^2, data=df)
x <- anova(m0)
ap <- qplot(p_reproduce, ave_cor, geom=c("boxplot"), data=df, xlab="Reproduction",ylab="")
bp <- qplot(p_selection, ave_cor, geom=c("boxplot"), data=df, xlab="Selection",ylab="")
cp <- qplot(n_offspring, ave_cor, geom=c("boxplot"), data=df, xlab="Offspring",ylab="")
dp <- qplot(distribution, ave_cor, geom=c("boxplot"), data=df, xlab="Distribution",ylab="")
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
@
...
Examining this further, factor-by-factor, where \textbf{Correlate Fidelity} is +1, from \cref{fig:lowstartfactorinfluence} \TODO{complete}.
...
<<distributionfunction, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Comparison of Gaussian and Uniform distributions for function $f()$.'>>=
df$distribution <- factor(df$distribution, labels=c("Gaussian","Uniform"))
ap <- qplot(distribution, ave_cor, geom="boxplot", data=df, xlab="",ylab="Final average fidelity")
bp <- qplot(distribution, ave_fit, geom="boxplot", data=df, xlab="",ylab="Final average fitness")
grid.arrange(ap,bp,nrow=1,ncol=2,heights=unit(0.5, "npc"))
@

\chapter[TODO-Test under fixed conditions]{TODO-Experimental test of hypothesis under fixed conditions}\label{experimental-test-of-h2-under-fixed-conditions}

Our experimental goals are to:

\begin{enumerate}
	\item Test the predictions of hypothesis \autoref{hypothesis-2}, and to 
	\item Examine the impact of the factors on the results.
\end{enumerate}

Taking these two together, we're conducting a form of exploration, and so would like a design that allows for a little open-ended questioning of the model. 

Although several parameters are discrete, at this stage the main ones can be reduced to a set of A/B alternatives, or in the language of experimental design, two-level factors. Our design should allow us to examine the effects of the factors, and also ideally their interactions, and so a form of factorial design is appropriate.

In fact we use a fractional factorial design to reduce the number of experimental runs required when compared to a full factorial design \autocite{Montgomery2009}. Although the number of runs is reduced, as each level of each factor occurs the same number of times in the results, the design remains ``balanced'' in the statistical sense, greatly easing analysis.

Hypothesis \autoref{hypothesis-2} makes two predictions for fixed environments: 
...
The Low-start case is the most significant of the two cases as it describes the core of the thesis - that good quality inheritance can develop from very imperfect beginnings, such as might be found during the transition from non-living to life. This makes it a key step along the way towards the evolution of evolution itself, the main subject of this thesis.

The first test therefore is to examine if inheritance emerges from low-fidelity/low-fitness initial conditions.

\section[Experiment 3]{Emergence of inheritance from Low-start initial conditions (experiment \#3)}

% EXPERIMENT 3 - Fidelity approaches 1.0
% Dataset: 2860d6fe, environment_change_frequency == 0, gen==500 (final results only), truncate==-1 (fitness-independent)

<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
temp <- load_df('results/results-2860d6fe.data')
df <- subset(temp, environment_change_frequency == 0 & gen==500 & truncate==0)
@
...
\subsection{Initial conditions}\label{factors-and-levels}
...
The design is complicated a little by the suspicion that two factors - $p_{reproduction}$ and $p_{selection}$ - require more than two levels. Fortunately a $2^n$ fractional design can be simply extended to include 4-level factors (\cite[368]{Montgomery2009}) either by replacement where each 4-level factor is mapped to two 2-level ones, or, as we choose to do, by simply running each combination of the four-level factors against a fractional-factorial design for the remaining factors in a hybrid full-fractional design.

\begin{table}
\begin{center}
\caption{Factor levels for experiment no.3}\label{tbl:factor_levels_3}
...
Based on preliminary runs, we begin with an initial estimate of 10 replicates for each combination of factor values given in the design; we confirm that this is sufficient through power calculations for specific tests where required in the later sections.

\subsection{Results and discussion}

Our interest is in the final values for fidelity under fixed-environment, Low-start, conditions. Therefore, data is from the final generation of those fixed-environment runs that reached completion at generation \Sexpr{max(df['gen'])} from the Low-start dataset; a total of \Sexpr{nrow(df)} runs out of the full dataset of \Sexpr{nrow(subset(temp,gen==0))} runs.

<<populationtrends, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Not all runs in fixed conditions maintain a substantial population over time.'>>=
ggplot(subset(temp, environment_change_frequency == 0 & truncate == 0)) + geom_jitter(aes(x=gen,y=pop))
@

%<<factorinfluences, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Influence of factors on population size at completion.'>>=
%ap <- qplot(p_reproduce, pop, geom=c("boxplot"), data=df, xlab="Reproduction",ylab="Population")
%bp <- qplot(p_selection, pop, geom=c("boxplot"), data=df, xlab="Selection",ylab="Population")
%cp <- qplot(n_offspring, pop, geom=c("boxplot"), data=df, xlab="Offspring",ylab="Population")
%dp <- qplot(distribution, pop, geom=c("boxplot"), data=df, xlab="Distribution",ylab="Population")
%grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
%@

\TODO{Which factor combinations lead to sustainable population? Is this the same as existing measure of reaching 500 generations?}

All of the runs that did not reach 500 generations had $p_{reproduction}=0.33$ and $n_{children}==2$. \Sexpr{nrow(subset(temp, p_reproduce == 1 & n_offspring==0 & gen==0))} runs of which \Sexpr{nrow(subset(temp, p_reproduce == 1 & n_offspring==0 & gen==500))} reached 500 generations, and are included in the next sample below. Of these runs that reached completion, all of those that in addition had factors $p_{selection}=0.66$ and Distribution $\mathcal{U}$
completed with much smaller populations (< \Sexpr{max(subset(temp,pop<1000)['pop'])}) than the runs which did not.

Of the \Sexpr{nrow(subset(df, gen==500 & pop < 4000))} runs that reached completion but with a substantially smaller population (< \Sexpr{max(subset(df, gen==500 & pop < 4000)['pop'])}) than the remaining runs (> \Sexpr{min(subset(df, gen=500 & pop >=4000)['pop'])}) it seems likely that none would continue much longer. All share the same combination of factors:

\begin{table}
	\begin{center}
		\caption{Common factor levels for small population}\label{tbl:factor_levels_small}
		\begin{tabular}{@{}llp{6cm}@{}}
			\toprule
			Parameter&  Number of Levels & Levels\\
			\midrule
			$p_{reproduction}$&       	1&	0.33\\
			$p_{selection}$&          	1&	0.66\\
			$n_{children}$&           	1&	2\\
			Starting population&		1&	Low-start settings\\
			\bottomrule
...
All runs with these factors either failed to reach completion, or reached completion with such a small population (< \Sexpr{max(subset(df, gen==500 & pop < 4000)['pop'])}) that they are extremely unlikely to continue, given the ever-downwards population trend. All other runs reached completion with a sustainable population.

\TODO{remove these factors from further analysis}

\TODO{results-2860d6fe.data dataset doesn't appear to have distribution==0 for these factors...unbalanced?}

<<lowstart, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Summary results for Low-start case, showing on the left-hand side an overview for the final average fidelity ($\\overline{fidelity}_{end}$) for each run and a density plot showing the distribution of $\\overline{fidelity}_{end}$. The right-hand side shows the corresponding plots for final average fitness ($\\overline{fitness}_{end}$)'>>=
...
<<lowstartfactors, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Comparison between results for correlated and uncorrelated factors. Top line shows results for the factor \\textbf{Correlate Fidelity} for final average fidelity ($\\overline{fidelity}_{end}$, on the left) and final average fitness ($\\overline{fitness}_{end}$, on right), while the bottom line the same for the factor \\textbf{Correlate Fitness}.'>>=
ap <- qplot(correlation_correlation, ave_cor, geom=c("boxplot","jitter"), data=df, xlab="",ylab="")
bp <- qplot(correlation_correlation, ave_fit, geom=c("boxplot","jitter"), data=df, xlab="",ylab="")
cp <- qplot(fitness_correlation, ave_cor, geom=c("boxplot","jitter"), data=df, xlab="",ylab="")
dp <- qplot(fitness_correlation, ave_fit, geom=c("boxplot","jitter"), data=df, xlab="",ylab="")
...
By inspection, all runs appear to have resulted in an increase in fidelity from the initial range of $[0, 0.3]$ (\cref{fig:lowstart}) but only some approach or reach 1.0. Those that did were uniformly associated with the \textbf{Correlate Fidelity} factor value of +1 (see top-left plot in \cref{fig:lowstartfactors}), and those that did not had a \textbf{Correlate Fidelity} value of -1. 

In conclusion, H$_0$ can be rejected, and H$_1$ accepted. Inheritance increases regardless of the model design, but is strongest when \textbf{Correlate Fidelity} is +1, that is, when the child's fidelity is correlated with that of its parent.

\section[Experiment 4]{Confirmation of inheritance under High-start conditions (experiment \#4)}

% EXPERIMENT 4 - Inheritance under High-start conditions
% Dataset: 2860d6fe-hs

<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df <- load_df('results/results-2860d6fe-hs.data')
df <- subset(df, truncate==0)
@

\begin{table}
	\begin{center}
		\caption{Factor levels for experiment no.4}\label{tbl:factor_levels_4}
		\begin{tabular}{@{}llp{6cm}@{}}
			\toprule
			Parameter&  Number of Levels & Levels\\
			\midrule
			...
			Correlate Fitness&        	2&	false or true\\
			Correlate Fidelity&       	2&	false or true\\
			Starting population&		1&	High-start settings\\
			\bottomrule
		\end{tabular}
	\end{center}
\end{table}

\section[Experiment 5]{Variance of Fidelity reduces under Low-start conditions (experiment \#5)}

% EXPERIMENT 5 - SD of Fidelity approaches 0
% Dataset: 2860d6fe, environment_change_frequency == 0, gen==500, truncate==0 (fitness-independent), runs that completed (that is, all replicates with same factors as a replicate which reached gen==500)

The second prediction of the hypothesis is that population variance for inheritance should decrease in fixed environments -- $\sigma_{fidelity}$, should decrease over time.
...
All settings are the same as for Experiment 3 (\cref{tbl:factor_levels_3}.)

\subsection{Results and discussion}

<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
temp <- load_df('results/results-2860d6fe.data')
full_factors <- unique(interaction(Filter(is.factor,temp)))
temp <- subset(temp, environment_change_frequency == 0 & truncate==0)
completing_factors <- unique(interaction(Filter(is.factor,temp[temp$gen==500,]))) # the factor levels that resulted in 500 generations
df <- temp[interaction(Filter(is.factor,temp)) %in% completing_factors,]
@

Data is from all generations of those fixed-environment runs that reached completion at generation \Sexpr{max(df['gen'])} from the Low-start dataset; a total of \Sexpr{nrow(subset(df,gen==0))} runs out of the full dataset of \Sexpr{nrow(subset(temp,gen==0))} runs.

%<<lowstartsdmean, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Ranges over time of the standard deviation (top row) of fidelity (left-hand side) and fitness (right-hand side) for both levels of \\textbf{Correlate Fidelity}. The bottom row shows the same for the mean rather than the standard deviation.'>>=
%ap <- ggplot(df,aes(x=gen,y=sd_cor,group=interaction(correlation_correlation), colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate Fidelity", values=c("grey10", "grey80")) + stat_smooth() + labs(x="Generation", y="Standard Deviation") + theme(legend.key=element_rect(fill="white"))
%bp <- ggplot(df,aes(x=gen,y=sd_fit,group=interaction(correlation_correlation),colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate Fidelity", values=c("grey10", "grey80")) + stat_smooth() + labs(x="Generation", y="Standard Deviation") + theme(legend.key=element_rect(fill="white"))
%cp <- ggplot(df,aes(x=gen,y=ave_cor,group=interaction(correlation_correlation), colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate Fidelity", values=c("grey10", "grey80")) + stat_smooth() + labs(x="Generation", y="Mean") + theme(legend.key=element_rect(fill="white"))
%dp <- ggplot(df,aes(x=gen,y=ave_fit,group=interaction(correlation_correlation), colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate Fidelity", values=c("grey10", "grey80")) + stat_smooth() + labs(x="Generation", y="Mean") + theme(legend.key=element_rect(fill="white"))
%grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2, right="Fitness", left="Fidelity", top="Standard Deviation", bottom="Mean")
%@

<<lowstartranges1, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Ranges over time of the standard deviation of fidelity (top) and fitness (bottom) for all levels of all factors.'>>=
ap <- ggplot(df,aes(x=gen,y=sd_cor,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution), colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate\nFidelity", values=c("grey80", "grey10"), labels=c("-1","+1")) + stat_smooth() + labs(x="Generation", y="Standard Deviation of Fidelity") + theme(legend.key=element_rect(fill="white"))
bp <- ggplot(df,aes(x=gen,y=sd_fit,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution),colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate\nFidelity", values=c("grey80", "grey10"), labels=c("-1","+1")) + stat_smooth() + labs(x="Generation", y="Standard Deviation of Fitness") + theme(legend.key=element_rect(fill="white"))
grid.arrange(ap,bp,nrow=2,ncol=1)
@

<<lowstartranges2, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='The same for the mean rather than the standard deviation - mean fidelity on the top row, mean fitness below.'>>=
cp <- ggplot(df,aes(x=gen,y=ave_cor,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution), colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate\nFidelity", values=c("grey80", "grey10"), labels=c("-1","+1")) + stat_smooth() + labs(x="Generation", y="Mean Fidelity") + theme(legend.key=element_rect(fill="white"))
dp <- ggplot(df,aes(x=gen,y=ave_fit,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution), colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate\nFidelity", values=c("grey80", "grey10"), labels=c("-1","+1")) + stat_smooth() + labs(x="Generation", y="Mean Fitness") + theme(legend.key=element_rect(fill="white"))
grid.arrange(cp,dp,nrow=2,ncol=1)
@

%<<lowstartranges2, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Range of 25\\% to 75\\% quartiles for \\gls{sd} for +1 setting for factor \\textbf{Correlate Fidelity}'>>=
...
\section[Experiment 6]{Confirmation that variance decreases under High-start conditions (experiment \#6)}
...
\subsection{Initial conditions}
All settings are the same as for Experiment 4 (\cref{tbl:factor_levels_4}.)

\subsection{Results and discussion}


\chapter[TODO-Test under changing conditions]{TODO-Experimental test of hypothesis under changing conditions}\label{experimental-test-of-h2-under-changing-conditions}
...
% Dataset: 2860d6fe, truncate==0 (fitness-independent), and correlation_correlation == 1

<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
temp <- load_df('results/results-2860d6fe.data')
df <- subset(temp, truncate==0 & correlation_correlation==1)
@
...
\begin{table}
	\begin{center}
		\caption{Factor levels for experiment no.8}\label{tbl:factor_levels_8}
		\begin{tabular}{@{}llp{6cm}@{}}
			\toprule
			Parameter&  Number of Levels & Levels\\
			\midrule
			...
			Correlate Fitness&        		2&	false or true\\
			Correlate Fidelity&       		2&	false or true\\
			Starting population&			2&	Low-start or High-start\\
			Environment change frequency	4&	0, or every 1, 5 or 10 generations\\
			\bottomrule
		\end{tabular}
	\end{center}
\end{table}
...
The \gls{sd} of the environmental change was set to 0.90 (parameter $sd$ to $TweakFitness$ equal to 0.90), and $meanOffset$, or the mean change to an individual's fitness as a result of the environmental change, to $-0.2$.  \TODO{Check these}

As before, each run has 10 replicates of \Sexpr{max(df['gen'])} generations each, with an initial population size of \Sexpr{df[1,]['pop']} entities.

Data is from the final generation of runs that reached completion at generation \Sexpr{max(df['gen'])} from the Low-start dataset; a total of \Sexpr{nrow(df)} runs out of the full dataset of \Sexpr{nrow(subset(temp,gen==0))} runs.

\section{Results and discussion}\label{results-8}

<<changingenvironmentsummary, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Final average fidelity (left) and fitness (right) over time for four different frequencies (0 or no change, every generation, every 5 generations, and every 10 generations) of environmental change.'>>=
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
ggplot(df) + geom_boxplot(aes(x=ecf,y=ave_cor)) + labs(x="Change Frequency", y="Final average fidelity")
ggplot(df) + geom_boxplot(aes(x=ecf,y=ave_fit)) + labs(x="Change Frequency", y="Final average fitness")
@

<<changingenvironmentmeans, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='The same for the mean rather than the standard deviation - mean fidelity on the top row, mean fitness below.'>>=
df <- subset(temp, truncate==0 & correlation_correlation==1)
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
cp <- ggplot(df) + stat_smooth(aes(x=gen,y=ave_cor,group=interaction(ecf), colour=factor(ecf))) + labs(x="Generation", y="Mean Fidelity") + theme(legend.key=element_rect(fill="white"))
dp <- ggplot(df) + stat_smooth(aes(x=gen,y=ave_fit,group=interaction(ecf), colour=factor(ecf))) + labs(x="Generation", y="Mean Fitness") + theme(legend.key=element_rect(fill="white"))
grid.arrange(cp,dp,nrow=2,ncol=1)
@

<<changingenvironmentpop, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='However, note that only some runs maintain their population size.'>>=
ggplot(df) + geom_jitter(aes(x=gen,y=pop, group=interaction(ecf),color=factor(ecf)))
@
...
<<gaussian, pdfcrop=TRUE, echo=FALSE, cache=TRUE, out.width='0.45\\linewidth',fig.show='hold'>>=
 ggplot(df_4915024_final, aes(x=distribution,y=ave_cor)) + geom_boxplot() + labs(x="Distribution", y="Final average fidelity")
 ggplot(df_4915024_final, aes(x=distribution,y=ave_fit)) + geom_boxplot() + labs(x="Distribution", y="Final average fitness")
@
...
Test: already shown in earlier analysis, specifically in the Hypothesis analysis sections where the factor \textbf{Correlate Fidelity} is set to the low value ("-1").
...
<<cyclesbypartition, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.cap='Cycles by reaction partition (starting reaction number for each partition along x-axis)'>>=
	df<-load_toyworld("results/EvaluatorActualCycles.out") 
