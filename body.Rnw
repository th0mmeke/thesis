...
% HIGHSTART
% 53f6b74 - 2860d6fe but ecf 0,1,5,10 <- MAIN DATASET FOR HIGHSTART
...
\begin{itemize}
	...
	\item \emph{Fidelity} measures the correlation between the child's value for a property and the same trait's value in the parent. The range is $[0,1]$ where a value of $0$ means that the value for a child's property has no correlation with its parent's value for that property. High \emph{fidelity} values mean high correlation, and when $fidelity = 1.0$ the child's value is identical to the parent's.  There is a subtle difference between fidelity and inheritance: inheritance is the goal, while fidelity is the mechanism, expressed as the degree of correlation between two generations.
\end{itemize}
...
\section[Upper size bound]{Effect of upper bound on population size}\label{check-effect-of-truncation}
...
The runs that reached completion are approximately evenly distributed between the two methods, fitness-independent (\Sexpr{nrow(df_completed[df_completed['truncate']==0,])} runs) and fitness-based (\Sexpr{nrow(df_completed[df_completed['truncate']==1,])} runs), indicating that the two methods at least appear comparable with respect to their affect on population size.
...
df <- subset(temp, environment_change_frequency == 0 & truncate==0)
@

We now turn to the other extreme - the effect of population extinctions. Runs in which the population dies out are incompatible with ongoing evolution, and so should be separated from the other, sustainable, runs in the analysis. This situation does arise in our experiments: not all experiment runs completed the target of \Sexpr{max(df['gen'])} generations (\cref{fig:populationtrends}), and not all of those that did, did so with a sustainable population size.

<<populationtrends, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Not all runs in fixed conditions can maintain a substantial population size over time.'>>=
ggplot(df) + geom_jitter(aes(x=gen,y=pop),size=0.25) + labs(x="Generation", y="Population size")
@

Of the \Sexpr{nrow(subset(df,gen==0))} experiment runs, \Sexpr{nrow(subset(df,gen==0))-nrow(subset(df, gen==500))} failed to reach completion, and \Sexpr{english(nrow(subset(df, gen==500 & pop < 4000)))} did so with a population size substantially smaller ($\leq$ \Sexpr{max(subset(df, gen==500 & pop < 4000)['pop'])}) than the population size in the other completed runs ($\geq$ \Sexpr{min(subset(df, gen==500 & pop >=4000)['pop'])}.) All other runs reached completion with a sustainable population size (in fact, with a population size at the upper bound of \Sexpr{min(subset(df, gen==500 & pop >=4000)['pop'])}.)

Inspecting the results by factor:
\begin{enumerate}
	\item Runs that fail to complete include $p_{reproduction}=0.33$ and $n_{children}=2$.
	\item Runs that complete, but with an unsustainably small population, also include $p_{selection}=0.66$. 
\end{enumerate}

From this we conclude:

\begin{enumerate}
	\item These results provide support for an experiment duration of \Sexpr{max(df['gen'])} generations - either a run maintains a stable population (in which case, the choice of duration is unrelated to population size considerations), or if a population is to go extinct it does so by \Sexpr{max(df['gen'])} generations or soon after and so can easily be identified for special treatment.
	\item Runs with factors associated with population extinction ($p_{reproduction}=0.33$, $n_{children}=2$ and $p_{selection}=0.66$) should be omitted from further analysis.
\end{enumerate}
...
ap <- ggplot(df) + geom_violin(aes(x=p_reproduce,y=ave_cor)) + labs(x="Reproduction",y="")
bp <- ggplot(df) + geom_violin(aes(x=p_selection,y=ave_cor)) + labs(x="Selection",y="")
cp <- ggplot(df) + geom_violin(aes(x=n_offspring,y=ave_cor)) + labs(x="Offspring",y="")
dp <- ggplot(df) + geom_violin(aes(x=distribution,y=ave_cor)) + labs(x="Distribution",y="")
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
@
% significant with F = \Sexpr{round(x['F value'],2)} and p-value \textless{} \Sexpr{round(x['Pr(>F)'],2)}.

Taking only runs where the factor \textbf{Correlate Fidelity} is +1, final average fidelity is significantly influenced by all factors and by the first-level interactions between $p_{reproduction}$ and $n_{children}$, and $p_{reproduction}$ and $Distribution$.
...
<<distributionfunction, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='h', fig.cap='Comparison of Gaussian and Uniform distributions for Factor Distribution.'>>=
df$distribution <- factor(df$distribution, labels=c("Gaussian","Uniform"))
...
<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df <- load_df('results/results-53f6b74.data')
...
\subsection{Initial conditions}
All settings are the same as in \cref{tbl:factor_levels_4}.
...
\begin{table}
	\begin{center}
		\caption{Factor levels for Changing Environments}\label{tbl:factor_levels_8}
		\begin{tabular}{@{}llp{6cm}@{}}
			\toprule
			Parameter&  Number of Levels & Levels\\
			\midrule
...
ggplot(df) + geom_violin(aes(x=ecf,y=ave_cor)) + labs(x="Change Frequency", y="Final average fidelity")
ggplot(df) + geom_violin(aes(x=ecf,y=ave_fit)) + labs(x="Change Frequency", y="Final average fitness")
@
...
df <- subset(temp, truncate==0 & correlation_correlation==1)
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
cp <- ggplot(df) + stat_smooth(aes(x=gen,y=ave_cor,group=interaction(ecf), colour=factor(ecf))) + scale_color_manual(name="Change frequency", values=c("grey70", "grey50", "grey30", "grey10")) + labs(x="Generation", y="Mean Fidelity") + theme(legend.key=element_rect(fill="white"))
dp <- ggplot(df) + stat_smooth(aes(x=gen,y=ave_fit,group=interaction(ecf), colour=factor(ecf))) + scale_color_manual(name="Change frequency", values=c("grey70", "grey50", "grey30", "grey10")) + labs(x="Generation", y="Mean Fitness") + theme(legend.key=element_rect(fill="white"))
grid.arrange(cp,dp,nrow=2,ncol=1)
@
...
\subsection[Choice of distribution function]{Is the choice of distribution function significant under changing conditions?}\label{is-the-choice-of-distribution-function-significant}
...
ggplot(df) + geom_boxplot(aes(x=distribution,y=ave_cor)) + labs(x="Distribution", y="Final average fidelity")
ggplot(df) + geom_boxplot(aes(x=distribution,y=ave_fit)) + labs(x="Distribution", y="Final average fitness")
@
