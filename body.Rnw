...
% 333345153 - folded vs non-folded distribution, with run tagging
% results-constant-change - same but steady selection pressure, with run tagging
...
<<changingenvironmentsd, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Standard deviation of fidelity over time for four different frequencies (0 or no change, every generation, every 5 generations, and every 10 generations) of environmental change.'>>=
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
#df_completed$temp <- factor(df_completed$truncate, labels=c("Independent","Dependent"))
df <- subset(df, truncate==0 & p_reproduce==0 & p_selection==0 & correlation_correlation==1 & fitness_correlation==1)
ggplot(df) + geom_point(aes(x=gen,y=sd_cor)) + facet_wrap(~ecf, ncol=2)  + labs(x="Generation", y="SD Fidelity") + coord_cartesian(ylim=c(0,1.1))
@

<<changingenvironmentmean, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Mean fitness (top) and mean fidelity (bottom) over time for four different frequencies (0 or no change, every generation, every 5 generations, and every 10 generations) of environmental change.'>>=
ap <- ggplot(df) + geom_point(aes(x=gen,y=ave_fit)) + facet_wrap(~ecf, ncol=2)  + labs(x="Generation", y="Mean Fitness") + coord_cartesian(ylim=c(0,1.1))
bp <- ggplot(df) + geom_point(aes(x=gen,y=ave_cor)) + facet_wrap(~ecf, ncol=2)  + labs(x="Generation", y="Mean Fidelity") + coord_cartesian(ylim=c(0,1.1))
grid.arrange(ap,bp,nrow=2,ncol=1)
@
...
Of the conceivable alternatives, there are three that appear worthy of further investigation:
...
<<summary-abrupt-change, pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df <- load_dfr('results/results-333345153.data')
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
df <- subset(df, truncate==0 & p_reproduce==0 & p_selection==0 & correlation_correlation==1 & fitness_correlation==1)
ap<-ggplot(df) + geom_line(aes(gen,ave_cor,group=run)) + facet_wrap(~ecf)
bp<-ggplot(df) + geom_line(aes(gen,sd_cor,group=run)) + facet_wrap(~ecf)
grid.arrange(ap,bp,nrow=2,ncol=1)
@

<<summary-constant-change, pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df <- load_dfr('results/results-constant-change.data')
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
df <- subset(df, truncate==0 & p_reproduce==0 & p_selection==0 & correlation_correlation==1 & fitness_correlation==1)
ap<-ggplot(df) + geom_line(aes(gen,ave_cor,group=run)) + facet_wrap(~ecf)
bp<-ggplot(df) + geom_line(aes(gen,sd_cor,group=run)) + facet_wrap(~ecf)
...
Appears to be a difference in response between the two different forms of environmental change. 
...
<<distributiontest, pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
