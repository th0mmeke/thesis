...
% LOWSTART
% 2860d6fe - Low-start with correct labels, ecf=0,1,5,10 <- MAIN DATASET
...
<<unboundedplot, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='On the left, a histogram of number of generations achieved by an unbounded algorithm before either i) the population size increases beyond 10x the original population size, or ii) the simulation succeeds in reaching the expected number of generations (which never occurred in an unbounded population). On the right, the growth of population size grouped by replicate (that is, by common factor levels) showing rapid population growth before reaching the 10x practicality limit.'>>=
...
<<limitcomparisonplots, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='Summary plots for fitness-independent sampling ("Independent") and fitness-based or dependent selection ("Dependent"), where the top line contains density plots and the bottom line boxplots showing ranges for both final average fidelity (on the left-hand side) and final average fitness (on the right).'>>=
...
<<limitqqplots, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='\\Gls{qq} plots comparing fitness-independent and fitness-based or dependent results for fidelity (left) and fitness (right).'>>=
...
\section{Hypothesis tests}\label{experimental-design}
...
<<lowstart, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='Summary results for Low-start case, showing on the left-hand side an overview for the final average fidelity ($\\overline{fidelity}_{end}$) for each run and a density plot showing the distribution of $\\overline{fidelity}_{end}$. The right-hand side shows the corresponding plots for final average fitness ($\\overline{fitness}_{end}$)'>>=
...
%<<lowstartsdmean, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='Ranges over time of the standard deviation (top row) of fidelity (left-hand side) and fitness (right-hand side) for both levels of \\textbf{Correlate Fidelity}. The bottom row shows the same for the mean rather than the standard deviation.'>>=
...
<<lowstartranges1, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='Ranges over time of the standard deviation of fidelity (top) and fitness (bottom) for all levels of all factors.'>>=
ap <- ggplot(df,aes(x=gen,y=sd_cor,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution), colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate\nFidelity", values=c("grey10", "grey80")) + stat_smooth() + labs(x="Generation", y="Standard Deviation of Fidelity") + theme(legend.key=element_rect(fill="white"))
bp <- ggplot(df,aes(x=gen,y=sd_fit,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution),colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate\nFidelity", values=c("grey10", "grey80")) + stat_smooth() + labs(x="Generation", y="Standard Deviation of Fitness") + theme(legend.key=element_rect(fill="white"))
grid.arrange(ap,bp,nrow=2,ncol=1)
@

<<lowstartranges2, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='The same for the mean rather than the standard deviation - mean fidelity on the top row, mean fitness below.'>>=
cp <- ggplot(df,aes(x=gen,y=ave_cor,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution), colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate\nFidelity", values=c("grey10", "grey80")) + stat_smooth() + labs(x="Generation", y="Mean Fidelity") + theme(legend.key=element_rect(fill="white"))
dp <- ggplot(df,aes(x=gen,y=ave_fit,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution), colour=factor(correlation_correlation))) + scale_color_manual(name="Correlate\nFidelity", values=c("grey10", "grey80")) + stat_smooth() + labs(x="Generation", y="Mean Fitness") + theme(legend.key=element_rect(fill="white"))
grid.arrange(cp,dp,nrow=2,ncol=1)
@

%<<lowstartranges2, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='Range of 25\\% to 75\\% quartiles for \\gls{sd} for +1 setting for factor \\textbf{Correlate Fidelity}'>>=
...
\chapter{TODO-Factor significance and interactions}
...
<<lowstartfactorinfluence, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='Influence of factors'>>=
m0 <- lm(ave_cor~(p_reproduce+p_selection+n_offspring+distribution+fitness_correlation)^2, data=df)
x <- anova(m0)
ap <- qplot(p_reproduce, ave_cor, geom=c("boxplot","jitter"), data=df, xlab="Reproduction",ylab="")
bp <- qplot(p_selection, ave_cor, geom=c("boxplot","jitter"), data=df, xlab="Selection",ylab="")
cp <- qplot(n_offspring, ave_cor, geom=c("boxplot","jitter"), data=df, xlab="Offspring",ylab="")
dp <- qplot(distribution, ave_cor, geom=c("boxplot","jitter"), data=df, xlab="Distribution",ylab="")
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
@
...
<<distributionfunction, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='Comparison of Gaussian and Uniform distributions for function $f()$.'>>=
...
<<changingenvironmentsummary, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='Fidelity (left) and fitness (right) over time for four different frequencies (0, or no change, every generation, every 5 generations, and every 10 generations) of environmental change.'>>=
