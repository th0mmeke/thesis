...
load_dfr <- function(t) {
	colClasses <- c("integer","numeric","numeric","integer","integer","numeric","numeric","factor","factor","factor","factor","factor","factor","factor","factor")
	read.csv(t, colClasses=colClasses)
}
...
\chapter{Initial screening experiment}

Translating model parameters into factors in the experiment design results in the factors in the first column of \ref{tbl:factor_levels_3}. As is usual with exploratory experiments with a number of parameters, where each run of the model has some cost in time or other resources, the key problem is to understand the relationship between parameters and response variables at an acceptable cost. In this case, our main cost is time - each run of an evolutionary model is cheap in resources but takes a little time. Exhaustively sampling the entire parameter space is unrealistic. Therefore, we first reduce the search space by limiting the number of values that each parameter can take. By choosing these values appropriately, we can construct an analysis model from the results that is sufficiently accurate for our exploratory purposes at a greatly reduced cost in time.
...
The factors and levels used are given in \ref{tbl:factor_levels_3}.
...
\section{Fitness and fidelity correlation}
...
\TODO{Remove fitness correlation?}
...
\TODO{Distribution - there is a difference, so either keep both, or just pick gaussian as uniform not very realistic}
...
ap <- ggplot(aggregate(df_d9f18e2_fixed$gen,Filter(is.factor,df_d9f18e2_fixed),max), aes(x))+geom_histogram(binwidth=1,origin = .5, fill='gray70') + labs(x="Final generation", y="Count of Runs") + scale_x_continuous(breaks=1:12) + scale_y_continuous(limits=c(0,2),breaks=c(0,1,2))
...
<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df_low <- load_df('results/results-53f6b74.data')
...
<<echo=FALSE>>=
runs <- nrow(subset(df_low,gen==0))
factors <- nrow(unique(Filter(is.factor,df_low)))
replicates <- runs / factors
@

Data is from \Sexpr{nrow(subset(df_low,gen==0))} runs -- \Sexpr{factors} unique sets of factors, each with \Sexpr{replicates} -- with settings given in \cref{tbl:factor_levels_ubmech} under fixed conditions, of which we are exclusively concerned with the subset of \Sexpr{nrow(df_completed)} runs that reached completion at \Sexpr{max(df_completed['gen'])} generations.
...
<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
temp <- load_df('results/results-53f6b74.data')
...
Fidelity correlation - main factor from hypothesis

Upper-size bound - sampling
Lower-size bound - sustainable populations

Probabilities - at least one of $p_{reproduction}$ and $p_{selection}$ non-zero.

Sensitivity to Distribution
Sensitivity to Number of offspring
Sensitivity to Correlate Fitness
...
As discussed earlier in section \cref{}, inheritance is the outcome of the relationship between parent and child traits, as represented by the measure of fidelity.
...
ap <- ggplot() + geom_point(data=s, aes(x=interaction,y=ave_cor, colour=factor(gen))) + scale_colour_manual(name="", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank(), legend.position='none')
bp <- ggplot() + geom_point(data=s, aes(x=interaction,y=ave_fit, colour=factor(gen))) + scale_colour_manual(name="", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fitness') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank(), legend.position='none')
cp <- ggplot() + geom_point(data=s, aes(x=interaction,y=sd_cor, colour=factor(gen))) + scale_colour_manual(name="", values=c("grey80", "grey10")) + labs(x='', y='SD fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank(), legend.position='none')
dp <- ggplot() + geom_point(data=s, aes(x=interaction,y=sd_fit, colour=factor(gen))) + scale_colour_manual(name="", values=c("grey80", "grey10")) + labs(x='', y='SD fitness') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank(), legend.position='none')
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
@
...
We use the same experimental setup and data as for the first hypothesis test in \cref{tbl:factor_levels_3}.
...
Fluctuating environment with no trend or direction--modelled by applying the distribution function from parameters (i.e., either gaussian or uniform) with a possible change to the mean fitness each generation. In this model, an individual's fitness may increase or
decrease.
...
The \gls{sd} of the environmental change was set to 0.90 (parameter $sd$ to $TweakFitness$ equal to 0.70), and $meanOffset$, or the mean change to an individual's fitness as a result of the environmental change, to $-0.2$.
...
Data is from those runs that reached completion at generation \Sexpr{max(df['gen'])} from the low-start dataset; a total of \Sexpr{nrow(df)} runs out of the full dataset of \Sexpr{nrow(subset(temp,gen==0))} runs.
...
%@
%
%<<changingenvironmentsd, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Standard deviation of final mean fidelity (top) and fitness (bottom) over time for four different frequencies (0 or no change, every generation, every 5 generations, and every 10 generations) of environmental change.'>>=
%ap <- ggplot(df) + stat_smooth(aes(x=gen,y=sd_cor, group=interaction(ecf), colour=factor(ecf))) + scale_color_manual(name="Change frequency", values=c("grey70", "grey50", "grey30", "grey10")) + labs(x="Generation", y="SD Fidelity") + theme(legend.key=element_rect(fill="white"))
%bp <- ggplot(df) + stat_smooth(aes(x=gen,y=sd_fit, group=interaction(ecf), colour=factor(ecf))) + scale_color_manual(name="Change frequency", values=c("grey70", "grey50", "grey30", "grey10")) + labs(x="Generation", y="SD Fitness") + theme(legend.key=element_rect(fill="white"))
%grid.arrange(ap,bp,nrow=2,ncol=1)
%@
%
...
<<changingenvironmentsd, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Standard deviation of fidelity (top) and mean fitness (bottom) over time for four different frequencies (0 or no change, every generation, every 5 generations, and every 10 generations) of environmental change.'>>=
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
ggplot(df) + stat_smooth(aes(x=gen,y=ave_fit)) + facet_wrap(~ecf, ncol=2)  + labs(x="Generation", y="Mean Fitness") + coord_cartesian(ylim=c(0,1.1))
ggplot(df) + stat_smooth(aes(x=gen,y=sd_cor)) + facet_wrap(~ecf, ncol=2)  + labs(x="Generation", y="SD Fidelity") + coord_cartesian(ylim=c(0,1.1))
@

<<changingenvironmentpop, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Issues with maintaining population size.'>>=
ggplot(df) + geom_point(aes(x=gen,y=pop),size=0.25) + labs(x="Generation", y="Population size") +  facet_wrap(~ecf, ncol=2)
@

We expect the variance of fidelity to remain at some non-zero level in changing environments, with greater change reflected in greater variance. This is not seen in the results. Instead we see a similar pattern to the variance in fixed environments where the variation approaches zero.

Does this represent a genuine difficulty with the hypothesis, or is there an alternative explanation?

Of the possible alternatives, four appear worth further investigation:

\begin{enumerate}
	\item The type of environmental change presented by the current $TweakFitness$ algorithm does not meet the precondition of a changing environment; there are however other forms of change that will show the hypothesised behaviour.
	...
	\item The shape of the distribution function is biasing results towards the outer limits of the range.
\end{enumerate}

<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
temp <- load_df('results/results-333345153.data')
df <- subset(temp, truncate==0 & correlation_correlation==1)
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
@

\subsection{Shape of Distribution produced by Derive parameter}\label{alternative-distribution}
...
ap <- qplot(x,geom='density')
bp <- qplot(sapply(sapply(x,min,1.0),max,0),geom='density')
...
cp <- qplot(x1,geom='density')
grid.arrange(ap,bp,cp,nrow=1,ncol=3)
@

This produces a distribution with fat tails (middle of \ref{fig:example_distributions}), like a 'W', rather than the expected elongated 'U' of the standard gaussian (on the left-hand side of the same figure), and as \emph{Derive} is used in \ref{base-model-algorithm} describes the relationship between the properties of an entity and its offspring, it seems plausible that this distortion towards the extremes of the range may have a significant effect on the experimental result.
	
To test this hypothesis, we compare the results obtained with the folding algorithm described above to those from a non-folding implementation (in ...) which produces the distribution shown on the right-hand side of \ref{fig:example_distributions}.

<<>>=
t <- load_dfr('results/results-333345153.data')
# test is if distribution affects sd_cor
ggplot(t) + geom_line(aes(gen,sd_cor,group=run)) + facet_wrap(~distribution)
ggplot(t) + geom_line(aes(gen,sd_cor,group=run)) + facet_wrap(~environment_change_frequency) # clear that horizontal lines where ecf=0
ggplot(subset(t,truncate==1 & distribution==0)) + geom_line(aes(gen,sd_cor,group=run)) + facet_wrap(~environment_change_frequency) # and only where ecf=0
@
<<>>=
ap <- ggplot(subset(t,distribution==1)) + geom_line(aes(gen,ave_cor,group=run)) + facet_wrap(~environment_change_frequency)
bp <- ggplot(subset(t,distribution==1)) + geom_line(aes(gen,sd_cor,group=run)) + facet_wrap(~environment_change_frequency)
grid.arrange(ap,bp,nrow=2,ncol=1)
@
...
From this it is clear that although distribution is important, it is not a sufficient explanation for the discrepancy.

\subsection{Effect of shape of environment change}

Previous experiments used an abrupt change in fitness at a specified frequency. This alternative instead uses a constant, smaller, change to simulate a constant selective pressure. \TODO{Is this what the hypothesis means though? Does it mean uncertainty/unpredictable change, or predictable - this model?}

<<>>=
t <- load_dfr('results/results-constant-change.data')
ggplot(t) + geom_line(aes(gen,ave_cor,group=run)) + facet_wrap(~environment_change_frequency)
@

Appears to be a difference in response to different frequencies from previous mechanism. Faceting by distribution, as in \ref{alternative-distribution}, confirms that the results under this form of environmental change remain sensitive to distribution.
