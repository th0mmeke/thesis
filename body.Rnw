...
As discussed earlier in section \cref{}, we treat inheritance as the relationship between parent and child traits, represented by our measure, fidelity.
...
ggplot(df) + stat_smooth(aes(x=gen,y=ave_fit)) + facet_wrap(~ecf, ncol=2)  + labs(x="Generation", y="Mean Fitness")
ggplot(df) + stat_smooth(aes(x=gen,y=sd_cor)) + facet_wrap(~ecf, ncol=2)  + labs(x="Generation", y="SD Fidelity")
@

<<changingenvironmentpop, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Issues with maintaining population size.'>>=
ggplot(df) + geom_jitter(aes(x=gen,y=pop),size=0.25) + labs(x="Generation", y="Population size") +  facet_wrap(~ecf, ncol=2)
@
