...
ap <- ggplot(df,aes(x=gen,y=sd_cor,group=interaction(correlation_correlation))) + stat_smooth() + labs(x="Generation", y="Standard Deviation")
bp <- ggplot(df,aes(x=gen,y=sd_fit,group=interaction(correlation_correlation))) + stat_smooth() + labs(x="Generation", y="Standard Deviation")
cp <- ggplot(df,aes(x=gen,y=ave_cor,group=interaction(correlation_correlation))) + stat_smooth() + labs(x="Generation", y="Mean")
dp <- ggplot(df,aes(x=gen,y=ave_fit,group=interaction(correlation_correlation))) + stat_smooth() + labs(x="Generation", y="Mean")
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2, right="Fitness", left="Fidelity", top="Standard Deviation", bottom="Mean")
@

<<lowstartranges, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold', fig.keep='last', fig.cap='Ranges over time of the standard deviation (top row) of fidelity (left-hand side) and fitness (right-hand side) for all levels of all factors. The bottom row shows the same for the mean rather than the standard deviation.'>>=
ap <- ggplot(df,aes(x=gen,y=sd_cor,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution))) + stat_smooth() + labs(x="Generation", y="Standard Deviation")
bp <- ggplot(df,aes(x=gen,y=sd_fit,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution))) + stat_smooth() + labs(x="Generation", y="Standard Deviation")
cp <- ggplot(df,aes(x=gen,y=ave_cor,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution))) + stat_smooth() + labs(x="Generation", y="Mean")
dp <- ggplot(df,aes(x=gen,y=ave_fit,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution))) + stat_smooth() + labs(x="Generation", y="Mean")
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2, right="Fitness", left="Fidelity", top="Standard Deviation", bottom="Mean")
@
