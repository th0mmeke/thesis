...
# CORRELATED,N_OFFSPRING,P_REPRODUCE,P_SELECTION,RESTRICTION,ave_cor,ave_fit,experiment,gen,pop,run,sd_cor,sd_fit
load.results.varying <- function(t) {
	colClasses <- c("factor","factor","factor","factor","factor","numeric","numeric","factor","factor","integer","factor","numeric","numeric")
	read.csv(t, colClasses=colClasses)
}
...
The major division is between living and artificial systems.
...
The previous sections have established the context, and identified the primary areas of relevant literature. This section now expands upon previous work that is directly relevant to our specific problem, the onset of evolution in artificial systems.
...
\section{Thought experiments and models}
Non-domain specific
\begin{itemize}
...
\section{Artificial Life}\label{artificial-life}
