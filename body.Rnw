<<setup, include=FALSE>>=
 ...
 df_d9f18e2 <- load_dfb('results/results-d9f18e2.data') # unbounded dataset
 df_d9f18e2_fixed <- df_d9f18e2[df_d9f18e2$environment_change_frequency==0,]
 df_low <- readRDS('results/results-dbb85db63.rds')
 df <- subset(df_low, environment_change_frequency==0 & gen==500) # all low-start, fixed environment runs that completed, and only the last generation
 completing_factors <- unique(interaction(Filter(is.factor,df_low[df_low$gen==500,]))) # the factor levels that resulted in 500 generations
 df_complete <- df_low[interaction(Filter(is.factor,df_low)) %in% completing_factors,] # raw data in long format - all low-start, fixed environment runs that completed
@

% df_4915024 <- load_dfb('results/results-4915024.data')
% df_041b62df <- load_dfb('results/results-041b62df.data')
% df_e22448db3 <- load_dfb('results/results-e22448db3.data')
% df_e124ce423 <- load_dfb('results/results-e124ce423.data')
% levels(df_041b62df$p_reproduce) <- c(0,1) 
% levels(df_041b62df$p_selection) <- c(0.33,0.66)
% levels(df_e22448db3$p_reproduce) <- c(0.33,0.66)
% levels(df_e22448db3$p_selection) <- c(0,1)
% levels(df_e124ce423$p_reproduce) <- c(0.33,0.66)
% levels(df_e124ce423$p_selection) <- c(0.33,0.66)
% levels(df_4915024$p_reproduce) <- c(0,1)
% levels(df_4915024$p_selection) <- c(0,1)
% saveRDS(rbind(df_4915024,df_e124ce423, df_e22448db3, df_041b62df),'results/low.rds')
% df_0101 <- load_dfb('results/results-0101.data')
% df_013366 <- load_dfb('results/results-013366.data')
% df_336601 <- load_dfb('results/results-336601.data')
% df_33663366 <- load_dfb('results/results-33663366.data')
% levels(df_013366$p_reproduce) <- c(0,1) 
% levels(df_013366$p_selection) <- c(0.33,0.66)
% levels(df_336601$p_reproduce) <- c(0.33,0.66)
% levels(df_336601$p_selection) <- c(0,1)
% levels(df_33663366$p_reproduce) <- c(0.33,0.66)
% levels(df_33663366$p_selection) <- c(0.33,0.66)
% levels(df_0101$p_reproduce) <- c(0,1)
% levels(df_0101$p_selection) <- c(0,1)
% saveRDS(rbind(df_0101,df_013366, df_336601, df_33663366),'results/dbb85db63.rds')
...
% change factor p_reproduce and p_selection to four way factors - with levels from runner.py
% e.g., currently levels(df_e124ce423$p_reproduce) = -1, 1 => 0.33, 0.66 and df_4915024467 => 0, 1
% Must change the encoding of the levels rather than simply labels

% Experiment 1 - No truncation/limit vs limits - d9f18e2
% Experiment 2 - Fitness-independent vs fitness-dependent, ecf=0, 491502446 onwards
% Experiment 3 - With truncation, Low start, ecf=0, all factors (p 0.33/0.66) - df_lowstart
% Experiment 4 - same as 3
% Experiment 5 - same as 3
% Highstart experiments - need equivalent of df_lowstart
...
Life (:qsee later) provides the canonical example of ``interesting'';
...
To test our hypothesis, we turn to experiment. If a simulation that accurately represents the system in the hypothesis behaves in a way that matches our predictions, the hypothesis is supported. On the other hand, if the behaviour doesn't align with the predictions, our hypothesis will be rejected. 
...
Our goal for this first set of experiments is to test the predictions of hypothesis \autoref{hypothesis-2} in a fixed environment (where the fitness of an entity does not change). To do so, we follow a five-step process:

\begin{enumerate}
	\item Develop a model of the hypothesis (\cref{base-model}.)
	\item Determine initial conditions (\cref{initial-conditions}.)
	\item Clarify any optional parameters (\cref{check-effect-of-truncation}.)
	\item Design an appropriate experiment (\cref{experimental-design}.)
	\item Conduct the experiment, and analyse the results (\cref{analysis}.)
\end{enumerate}

\section{Base model}\label{base-model}

All experiments in this section make use of the same base model (\autoref{base-model-algorithm}) where the key elements of the hypothesis, such as inheritance fidelity, are explicit parameters in the model. The main elements will be quite familiar to anyone from Evolutionary Computation or Evolutionary Biology, although the introduction of fidelity is novel and important to the overall thesis.
...
\subsection{Initial conditions for upper bound investigation}

The environment is considered ``fixed'' (that is, each element's fitness value does not change); the initial population consists of \rinline{df_d9f18e2[1,]$pop} elements constructed according to the Low Start case (\cref{low-start-case}) and the maximum number of generations equals \rinline{max(df_d9f18e2$gen)} generations.

\subsection{Unbounded populations}

% EXPERIMENT 1 - UNBOUNDED POPULATIONS

<<unboundedplot, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='On the left, a histogram of number of generations achieved by an unbounded algorithm before either i) the population size increases beyond 10x the original population size, or ii) the simulation succeeds in reaching the expected number of generations (which never occurred in an unbounded population). On the right, the growth of population size grouped by replicate (that is, by common factor levels) showing rapid population growth before reaching the 10x practicality limit.'>>=
 attach(df_d9f18e2_fixed)
 df_max <- aggregate(df_d9f18e2_fixed$gen,list(p_reproduce, p_selection,n_offspring,truncate,distribution,fitness_correlation, correlation_correlation, environment_change_frequency),max)
 detach(df_d9f18e2_fixed)
 ap <- ggplot(df_max, aes(x))+geom_histogram(binwidth=1,origin = .5) + labs(x="Final generation", y="Count of Runs") + scale_x_continuous(breaks=1:12) + scale_y_continuous(limits=c(0,2),breaks=c(0,1,2))
...
\subsection{Is the choice of mechanism significant?}
...
We use a standard two-sample Kolmogorov-Smirnov test to test the similarity between the distributions.
...
<<echo=FALSE>>=
 df_fixed <- subset(df_low, environment_change_frequency == 0)
 df_completed <- subset(df_fixed, gen==500)
@

Data is taken from the Low Start dataset, made up of \rinline{nrow(na.omit(df_fixed))} runs under fixed conditions, of which we are exclusively concerned with the subset of \rinline{nrow(na.omit(df_completed))} runs that reached completion at \rinline{max(df_d9f18e2$gen)} generations.

The runs that reached completion are approximately evenly distributed between the two methods, fitness-independent (\rinline{nrow(df_completed[df_completed$truncate==-1,])} runs) and fitness-based (\rinline{nrow(df_completed[df_completed$truncate==1,])}), indicating that the two methods at least appear comparable with respect to their affect on population size.
...
<<limitcomparisonplots, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='Summary plots for fitness-independent sampling ("Independent") and fitness-based or dependent selection ("Dependent"), where the top line contains density plots and the bottom line boxplots showing ranges for both final average fidelity (on the left-hand side) and final average fitness (on the right).'>>=
...
<<limitqqplots, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='\\Gls{qq} plots comparing fitness-independent and fitness-based or dependent results for fidelity (left) and fitness (right).'>>=
...
Applying a two-sample Kolmogorov-Smirnov test to determine if the results for each method are taken from the same distribution provides strong evidence (for population mean fidelity, approx. p-value=\rinline{round(x_cor$p.value,2)} and for pop. mean fitness, approx. p-value=\rinline{round(x_fit$p.value,2)}) to reject the null hypothesis, confirmed by visual inspection of \cref{fig:limitcomparisonplots} and \cref{fig:limitqqplots}. The distributions produced by the two methods are different for both average fidelity and average fitness.
...
Our experimental goals are to assess the hypothesis and to examine the influence of factors - essentially, we're conducting a form of exploration, and so would like a design that allows for a little open-ended questioning of the model. Although several parameters are discrete, at this stage the main ones can be reduced to a set of A/B alternatives, or in the language of experimental design, two-level factors. Our design should allow us to examine the effects of the factors, and also ideally their interactions, and so a form of factorial design is appropriate.
...
From the predictions of hypothesis \autoref{hypothesis-2} (\cref{predictions}) the main property of interest is \emph{Fidelity}, or the correlation between parent and child property values. \emph{Fidelity} therefore is our response variable. 

Specifically we use $\overline{fidelity}_{end}$, or the mean value for \emph{Fidelity} (across all replicates) at the end of a run, as under hypothesis \autoref{hypothesis-2} we expect $\overline{fidelity}_{end}$ to approach 1.0 in an unchanging environment. \TODO{ need this?} In addition, as we predict that \emph{Fidelity} will increase, we expect $\overline{fidelity}_{end}--\overline{fidelity}_{start} > 0$.
...
Population Restriction&  	2&	false or true\\
...
\section{Results and discussion}\label{analysis}
...
\begin{enumerate}
	\item Experimentally test the predictions of hypothesis \autoref{hypothesis-2}, and 
...
Hypothesis \autoref{hypothesis-2} makes two predictions when there is a fixed environment: 
...
<<lowstart, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='Summary results for Low Start case, showing on the left-hand side an overview for the final average fidelity ($\\overline{fidelity}_{end}$) for each run and a density plot showing the distribution of $\\overline{fidelity}_{end}$. The right-hand side shows the corresponding plots for final average fitness ($\\overline{fitness}_{end}$)'>>=
...
By inspection, all runs appear to have resulted in an increase in fidelity from the initial range of $[0, 0.3]$ (\cref{fig:lowstart}) but only some approach or reach 1.0. Those that did were uniformly associated with the \textbf{Correlate Fidelity} factor value of +1 (see top-left plot in \cref{fig:lowstartcorrelation}), and those that did not had a \textbf{Correlate Fidelity} value of -1. 

In conclusion, H$_0$ can be rejected, and H$_1$ accepted. Inheritance increases regardless of the model design, but is strongest when \textbf{Correlate Fidelity} is +1, that is, correlated.

The second test examines the change in the \gls{sd} of \emph{Fidelity} -- by hypothesis, in an unchanging environment, $\sigma{fidelity}$ should decrease over time.
...
\TODO{make sure ranges are correct in figure lowstartsdmean}
<<lowstartsdmean, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='Ranges over time of the standard deviation (top row) of fidelity (left-hand side) and fitness (right-hand side) for both levels of \\textbf{Correlate Fidelity}. The bottom row shows the same for the mean rather than the standard deviation.'>>=
ap <- ggplot(dfa,aes(x=gen,y=sd_cor,group=interaction(correlation_correlation))) + stat_smooth() + labs(x="Generation", y="Standard Deviation")
bp <- ggplot(dfa,aes(x=gen,y=sd_fit,group=interaction(correlation_correlation))) + stat_smooth() + labs(x="Generation", y="Standard Deviation")
cp <- ggplot(dfa,aes(x=gen,y=ave_cor,group=interaction(correlation_correlation))) + stat_smooth() + labs(x="Generation", y="Mean")
dp <- ggplot(dfa,aes(x=gen,y=ave_fit,group=interaction(correlation_correlation))) + stat_smooth() + labs(x="Generation", y="Mean")
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2, right="Fitness", left="Fidelity", top="Standard Deviation", bottom="Mean")
@

<<lowstartranges, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='Ranges over time of the standard deviation (top row) of fidelity (left-hand side) and fitness (right-hand side) for all levels of all factors. The bottom row shows the same for the mean rather than the standard deviation.'>>=
ap <- ggplot(dfa,aes(x=gen,y=sd_cor,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,truncate,distribution))) + stat_smooth() + labs(x="Generation", y="Standard Deviation")
bp <- ggplot(dfa,aes(x=gen,y=sd_fit,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,truncate,distribution))) + stat_smooth() + labs(x="Generation", y="Standard Deviation")
cp <- ggplot(dfa,aes(x=gen,y=ave_cor,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,truncate,distribution))) + stat_smooth() + labs(x="Generation", y="Mean")
dp <- ggplot(dfa,aes(x=gen,y=ave_fit,group=interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,truncate,distribution))) + stat_smooth() + labs(x="Generation", y="Mean")
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2, right="Fitness", left="Fidelity", top="Standard Deviation", bottom="Mean")
@



<<lowstartranges2, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='Range of 25% to 75% quartiles for \\gls{sd} for +1 setting for factor \\textbf{Correlate Fidelity}'>>=
 y <- df_complete
 #z <- by(y,y$gen,function(x) {aggregate(x$sd_cor, Filter(is.factor,x), quantile)}) # for each generation, get quantiles for sd_cor for each factor combination
 z<-by(y,y$gen,function(x) {aggregate(x$sd_cor ~ x$correlation_correlation, data=x, quantile)}) # same, but just aggregated by correlation_correlation
 ...
 z2<-z1[1:3,1:501]
 z3<-cbind(1:501,z2[1,],z2[2,],z2[3,])
z4 <- as.data.frame(z3)
 ggplot(z4,aes(V1,V2,V3)) + geom_ribbon(data=z4,aes(ymin=V2,ymax=V4), alpha = 0.2) + geom_line(aes(V1,V3))
@

\subsubsection{Factor significance and interactions}

<<lowstartfactorinfluence, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='.'>>=
 df_test <- df[df$correlation_correlation == 1,]
 m0 <- lm(ave_cor~(p_reproduce+p_selection+n_offspring+distribution+fitness_correlation)^2, data=df_test)
 x <- anova(m0)
 ap <- qplot(p_reproduce, ave_cor, geom=c("boxplot","jitter"), data=df_test, xlab="Reproduction",ylab="")
 bp <- qplot(p_selection, ave_cor, geom=c("boxplot","jitter"), data=df_test, xlab="Selection",ylab="")
 cp <- qplot(n_offspring, ave_cor, geom=c("boxplot","jitter"), data=df_test, xlab="Offspring",ylab="")
 dp <- qplot(distribution, ave_cor, geom=c("boxplot","jitter"), data=df_test, xlab="Distribution",ylab="")
 grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
@
...
<<distributionfunction, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='Comparison of Gaussian and Uniform distributions for function $f()$.'>>=
...
#x <- power.anova.test(groups=2,n=dim(df_high)[1],between.var=between,within.var=within)
@

% The statistical power of the design is \rinline{x['power']} with \rinline{x['groups']} groups and \rinline{x['n']} observations per group.
...
\begin{table}
	\begin{center}
		...
		\begin{tabular}{@{}llllp{5cm}@{}}
			...
			4&  $f$&                      			-1&	Gaussian dist., $\mathbb{N}$&          Function for generating a sample value from a particular distribution\\
...
<<changingenvironmentsummary, echo=FALSE, fig.show='hold', fig.keep='last', fig.cap='Fidelity (left) and fitness (right) over time for four different frequencies (0, or no change, every generation, every 5 generations, and every 10 generations) of environmental change.'>>=
 ecf <- factor(df_4915024_final$environment_change_frequency,levels=c(0,1,5,10))
 ggplot(df_4915024_final, aes(x=ecf,y=ave_cor)) + geom_boxplot() + labs(x="Change Frequency", y="Final average fidelity")
 ggplot(df_4915024_final, aes(x=ecf,y=ave_fit)) + geom_boxplot() + labs(x="Change Frequency", y="Final average fitness")
@
...
<<gaussian, echo=FALSE, out.width='0.45\\linewidth',fig.show='hold'>>=
...
\begin{figure}[htb]
	...
	\caption{Example molecules generated by ToyWorld, rendered as 2-D objects from the original SMILES notation - [H][C]N(O[N])OOC([H])(O[H])ON([O])[N][N]N(O[O])OOO[H] (top left), [H]OON(O[N+]([O-])[N+]([O])[O-])N([H])O[O] (top right), [H]OOON([H])ON([H])O[H] (bottom left) and [O]C(=O)OOOON=[C]N([O])[O]  (bottom right)}\label{fig:toyworld-example-molecules}
\end{figure}
...
\TODO{convert to ggplot2}
<<cyclesbypartition, echo=FALSE, fig.show='hold', fig.cap='Cycles by reaction partition (starting reaction number for each partition along x-axis)'>>=
...
\TODO{convert to ggplot2}
<<reactantstrategy, echo=FALSE, fig.show='hold', fig.cap='Response by Reactant Strategy '>>=
...
\TODO{convert to ggplot2}
<<productstrategy, echo=FALSE, fig.show='hold', fig.cap='Response by Product Strategy '>>=
	par(mfrow=c(1,3),cex=0.5)
...
\TODO{convert to ggplot2}
<<reactantproductcombination, echo=FALSE, out.width='0.30\\linewidth', fig.height=4, fig.show='hold', fig.cap='Effect of the combination of Reactant and Product Strategies on Response Variables '>>=
