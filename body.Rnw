...
Data is from \Sexpr{nrow(subset(df_low,gen==0))} runs with settings given in \ref{tbl:factor_levels_ubmech} under fixed conditions, of which we are exclusively concerned with the subset of \Sexpr{nrow(df_completed)} runs that reached completion at \Sexpr{max(df_completed['gen'])} generations.
