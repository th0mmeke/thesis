...
<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df <- subset(load_df('results/results-2860d6fe.data'), environment_change_frequency == 0 & gen==500 & truncate==0)
...
<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df <- subset(load_df('results/results-53f6b74.data'), environment_change_frequency == 0 & gen==500 & truncate==0)
df <- subset(df, pop > 1000) # only "sustainable" populations - see discussion on lower-bounds
@
...
ap <- ggplot(df) + geom_box(aes(x=p_reproduce,y=ave_cor)) + labs(x="Reproduction",y="Final mean fidelity")
bp <- ggplot(df) + geom_box(aes(x=p_selection,y=ave_cor)) + labs(x="Selection",y="Final mean fidelity")
cp <- ggplot(df) + geom_box(aes(x=n_offspring,y=ave_cor)) + labs(x="Offspring",y="Final mean fidelity")
dp <- ggplot(df) + geom_box(aes(x=distribution,y=ave_cor)) + labs(x="Distribution",y="Final mean fidelity")
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
@
...
df$distribution <- factor(df$distribution, labels=c("Gaussian","Uniform"))
ap <- ggplot(df) + geom_box(aes(x=distribution, y=ave_cor)) + labs(x="",y="Final mean fidelity")
bp <- ggplot(df) + geom_box(aes(x=distribution, y=ave_fit)) + labs(x="",y="Final mean fitness")
grid.arrange(ap,bp,nrow=1,ncol=2,heights=unit(0.5, "npc"))
@
