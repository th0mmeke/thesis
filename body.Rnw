...
\chapter{Model and initial factor screening}
...
All experiments in this part of the work make use of the same base model (\autoref{base-model-algorithm}) where the key elements of the hypothesis, such as inheritance fidelity, are represented as explicit parameters. The main elements will be quite familiar to anyone from Evolutionary Computation or Evolutionary Biology, although the introduction of fidelity is novel and important to the overall thesis.
...
The environment is considered ``fixed'' (that is, each element's fitness value does not change); the initial population consists of \Sexpr{df_d9f18e2[1,]['pop']} elements constructed according to the low-start case (\cref{low-start-case}), and the model run with a maximum of \Sexpr{max(df_d9f18e2['gen'])} generations with 

\begin{table}
	\begin{center}
		\caption{Settings for upper-bound investigation}\label{tbl:factor_levels_ub}
...
\begin{table}
	\begin{center}
		\caption{Settings for the investigation into an upper-bound mechanism}\label{tbl:factor_levels_ubmech}
		\begin{tabular}{@{}llp{6cm}@{}}
			\toprule
			Parameter&  Number of Levels & Levels\\
			\midrule
			...
			Correlate Fitness&        	2&	false or true\\
			Correlate Fidelity&       	2&	false or true\\
			Starting population&		1&	Low-start settings, 5000 elements\\
			Environment&				1&	Fixed\\
			\bottomrule
		\end{tabular}
	\end{center}
...
Data is from \Sexpr{nrow(subset(df_low,gen==0))} runs with settings given in \cref{tbl:factor_levels_ubmech} under fixed conditions, of which we are exclusively concerned with the subset of \Sexpr{nrow(df_completed)} runs that reached completion at \Sexpr{max(df_completed['gen'])} generations.
...
Returning to the overall goals for these experiments, to recap we wish to:
...
The first test therefore is to examine if inheritance emerges from low-fidelity and low-fitness initial conditions.
...
<<ExperimentInheritanceFromLowStart, pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df <- subset(load_df('results/results-2860d6fe.data'), environment_change_frequency == 0 & truncate==0)
df <- subset(df, pop > 1000) # only "sustainable" populations - see discussion on lower-bounds

df$interaction <- with(df,interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution))
s <- data.frame(interaction =unique(with(df,interaction(p_reproduce,p_selection,fitness_correlation,correlation_correlation,distribution))))
s0 <- data.frame(gen='Initial', aggregate(sd_cor~interaction,subset(df,gen==0),mean),aggregate(sd_fit~interaction,subset(df,gen==0),mean), aggregate(ave_cor~interaction,subset(df,gen==0),mean), aggregate(ave_fit~interaction,subset(df,gen==0),mean) )
s500 <- data.frame(gen='Final', aggregate(sd_cor~interaction,subset(df,gen==500),mean),aggregate(sd_fit~interaction,subset(df,gen==500),mean), aggregate(ave_cor~interaction,subset(df,gen==500),mean), aggregate(ave_fit~interaction,subset(df,gen==500),mean) )
s<- rbind(s0,s500)
s$gen <- as.factor(s$gen)

df <- subset(df, gen==500) # final results
@
...
\begin{table}
\begin{center}
\caption{Factors and levels for inheritance under low-start conditions}\label{tbl:factor_levels_3}
\begin{tabular}{@{}llp{6cm}@{}}
\toprule
...
<<lowstartbyfactor, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap=''>>=
ap <- ggplot() + geom_point(data=s, aes(x=interaction,y=ave_cor, colour=factor(gen))) + scale_colour_manual(name="Generation", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank()) 
bp <- ggplot() + geom_point(data=s, aes(x=interaction,y=ave_fit, colour=factor(gen))) + scale_colour_manual(name="Generation", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank())
cp <- ggplot() + geom_point(data=s, aes(x=interaction,y=sd_cor, colour=factor(gen))) + scale_colour_manual(name="Generation", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank()) 
dp <- ggplot() + geom_point(data=s, aes(x=interaction,y=sd_fit, colour=factor(gen))) + scale_colour_manual(name="Generation", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank())  
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
@
...
<<lowstartfactors, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Comparison between results for correlated and uncorrelated factors. Top line shows results for the factor \\textbf{Correlate Fidelity} for final mean fidelity ($\\overline{fidelity}_{end}$, on the left) and final mean fitness ($\\overline{fitness}_{end}$, on right), while the bottom line the same for the factor \\textbf{Correlate Fitness}.'>>=
ap <- ggplot(df) + geom_boxplot(aes(x=correlation_correlation,y=ave_cor)) + labs(x='',y='')
bp <- ggplot(df) + geom_boxplot(aes(x=correlation_correlation,y=ave_fit)) + labs(x='',y='')
cp <- ggplot(df) + geom_boxplot(aes(x=fitness_correlation,y=ave_cor)) + labs(x='',y='')
dp <- ggplot(df) + geom_boxplot(aes(x=fitness_correlation,y=ave_fit)) + labs(x='',y='')
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2, right="Final mean fitness", left="Final mean fidelity", top="Factor Correlate Fidelity", bottom="Factor Correlate Fitness")
@
...
<<ExperimentInheritanceFromHighStart, pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df <- subset(load_df('results/results-53f6b74.data'), environment_change_frequency == 0 & truncate==0)
df <- subset(df, pop > 1000) # only "sustainable" populations - see discussion on lower-bounds
...
#s <- merge(s, data.frame(aggregate(sd_cor~interaction,subset(df,gen==500),mean)), by='interaction', suffixes=c('.initial','.final'))
#s <- merge(s, data.frame(aggregate(sd_fit~interaction,subset(df,gen==0),mean)), by='interaction')
#s <- merge(s, data.frame(aggregate(sd_fit~interaction,subset(df,gen==500),mean)), by='interaction', suffixes=c('.initial','.final'))
#s <- merge(s, data.frame(aggregate(ave_cor~interaction,subset(df,gen==0),mean)), by='interaction')
#s <- merge(s, data.frame(aggregate(ave_cor~interaction,subset(df,gen==500),mean)), by='interaction', suffixes=c('.initial','.final'))
#s <- merge(s, data.frame(aggregate(ave_fit~interaction,subset(df,gen==0),mean)), by='interaction')
#s <- merge(s, data.frame(aggregate(ave_fit~interaction,subset(df,gen==500),mean)), by='interaction', suffixes=c('.initial','.final'))
@

\begin{table}
	\begin{center}
		\caption{Factors and levels for inheritance under high-start conditions}\label{tbl:factor_levels_4}
		\begin{tabular}{@{}llp{6cm}@{}}
			\toprule
			Factor&  Number of Levels & Levels\\
			\midrule
...
<<highstartbyfactor, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap=''>>=
ap <- ggplot() + geom_point(data=s, aes(x=interaction,y=ave_cor, colour=factor(gen))) + scale_colour_manual(name="Generation", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank()) 
bp <- ggplot() + geom_point(data=s, aes(x=interaction,y=ave_fit, colour=factor(gen))) + scale_colour_manual(name="Generation", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank())
cp <- ggplot() + geom_point(data=s, aes(x=interaction,y=sd_cor, colour=factor(gen))) + scale_colour_manual(name="Generation", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank()) 
dp <- ggplot() + geom_point(data=s, aes(x=interaction,y=sd_fit, colour=factor(gen))) + scale_colour_manual(name="Generation", values=c("grey80", "grey10")) + labs(x='Factor combination', y='Mean fidelity') + theme(axis.ticks.x=element_blank(), axis.text.x = element_blank())  
...
<<highstartfactors, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Comparison between results for correlated and uncorrelated factors. Top line shows results for the factor \\textbf{Correlate Fidelity} for final mean fidelity ($\\overline{fidelity}_{end}$, on the left) and final mean fitness ($\\overline{fitness}_{end}$, on right), while the bottom line the same for the factor \\textbf{Correlate Fitness}.'>>=
ap <- ggplot(df) + geom_boxplot(aes(x=correlation_correlation,y=ave_cor)) + labs(x='',y='')
bp <- ggplot(df) + geom_boxplot(aes(x=correlation_correlation,y=ave_fit)) + labs(x='',y='')
cp <- ggplot(df) + geom_boxplot(aes(x=fitness_correlation,y=ave_cor)) + labs(x='',y='')
dp <- ggplot(df) + geom_boxplot(aes(x=fitness_correlation,y=ave_fit)) + labs(x='',y='')
...
<<ExperimentVarianceUnderLowStart, pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
temp <- load_df('results/results-2860d6fe.data')
df <- subset(df, pop > 1000) # only "sustainable" populations - see discussion on lower-bounds
full_factors <- unique(interaction(Filter(is.factor,temp)))
temp <- subset(temp, environment_change_frequency == 0 & truncate==0)
completing_factors <- unique(interaction(Filter(is.factor,temp[temp$gen==500 & temp$pop>1000,]))) # the factor levels that resulted in 500 generations with 'sustainable' population
df <- temp[interaction(Filter(is.factor,temp)) %in% completing_factors,]
@

Data is all generations for those fixed-environment runs that reached completion with a 'sustainable' population (see \cref{lower-size-limit}) at generation \Sexpr{max(df['gen'])}; a total of \Sexpr{nrow(subset(df,gen==0))} runs out of the full dataset of \Sexpr{nrow(subset(temp,gen==0))} runs.

<<lowstartranges1, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Ranges over time of the standard deviation of fidelity (top) and fitness (bottom) for all levels of all factors, broken out by level of \\emph{Correlate Fidelity}.'>>=
...
<<lowstartranges2, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Ranges for the mean rather than the standard deviation - mean fidelity on the top row, mean fitness below - again by \\emph{Correlate Fidelity}.'>>=
...
%<<lowstartranges2, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Range of 25\\% to 75\\% quartiles for \\gls{sd} for +1 setting for factor \\textbf{Correlate Fidelity}'>>=
...
<<ExperimentVarianceUnderHighStart, pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
@
...
<<lowstartfactorinfluence, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Box-plots for the effect of each individual major factor on final mean fidelity.'>>=
...
Taking only runs where the factor \textbf{Correlate Fidelity} is +1, final mean fidelity is significantly influenced by all factors and by the first-level interactions between $p_{reproduction}$ and $n_{children}$, and $p_{reproduction}$ and $Distribution$.
...
<<changingenvironmentsummary, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Final mean fidelity for four different frequencies (0 or no change, every generation, every 5 generations, and every 10 generations) of environmental change.'>>=
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
ap <- ggplot(df) + geom_violin(aes(x=ecf,y=sd_cor)) + labs(x="Change Frequency", y="Final SD fidelity")
bp <- ggplot(df) + geom_violin(aes(x=ecf,y=sd_fit)) + labs(x="Change Frequency", y="Final SD fitness")
cp <- ggplot(df) + geom_violin(aes(x=ecf,y=ave_cor)) + labs(x="Change Frequency", y="Final mean fidelity")
dp <- ggplot(df) + geom_violin(aes(x=ecf,y=ave_fit)) + labs(x="Change Frequency", y="Final mean fitness")
grid.arrange(ap,bp,cp,dp,nrow=2,ncol=2)
@

<<changingenvironmentsd, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Final mean fidelity (left) and fitness (right) over time for four different frequencies (0 or no change, every generation, every 5 generations, and every 10 generations) of environmental change.'>>=
df <- subset(temp, truncate==0 & correlation_correlation==1)
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
ap <- ggplot(df) + stat_smooth(aes(x=gen,y=std_cor,group=interaction(ecf), colour=factor(ecf))) + scale_color_manual(name="Change frequency", values=c("grey70", "grey50", "grey30", "grey10")) + labs(x="Generation", y="SD Fidelity") + theme(legend.key=element_rect(fill="white"))
bp <- ggplot(df) + stat_smooth(aes(x=gen,y=std_fit,group=interaction(ecf), colour=factor(ecf))) + scale_color_manual(name="Change frequency", values=c("grey70", "grey50", "grey30", "grey10")) + labs(x="Generation", y="SD Fitness") + theme(legend.key=element_rect(fill="white"))
grid.arrange(ap,bp,nrow=2,ncol=1)
@

<<changingenvironmentmeans, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='The same for the mean rather than the standard deviation - mean fidelity on the top row, mean fitness below.'>>=
df <- subset(temp, truncate==0 & correlation_correlation==1)
df$ecf <- factor(df$environment_change_frequency,levels=c(0,1,5,10)) # to put into a natural order...
ap <- ggplot(df) + stat_smooth(aes(x=gen,y=ave_cor,group=interaction(ecf), colour=factor(ecf))) + scale_color_manual(name="Change frequency", values=c("grey70", "grey50", "grey30", "grey10")) + labs(x="Generation", y="Mean Fidelity") + theme(legend.key=element_rect(fill="white"))
bp <- ggplot(df) + stat_smooth(aes(x=gen,y=ave_fit,group=interaction(ecf), colour=factor(ecf))) + scale_color_manual(name="Change frequency", values=c("grey70", "grey50", "grey30", "grey10")) + labs(x="Generation", y="Mean Fitness") + theme(legend.key=element_rect(fill="white"))
grid.arrange(ap,bp,nrow=2,ncol=1)
@

<<changingenvironmentpop, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Issues with maintaining population size.'>>=
