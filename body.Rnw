...
\paragraph{Probability of selection and probability of reproduction}
...
The combination where both $p_{selection}$ and $p_{reproduction}$ are non-zero does however lead to uninteresting behaviour as the major source of variation has been removed from the model.

\paragraph{Fitness and fidelity correlation}
...
Data is from \Sexpr{runs} runs -- \Sexpr{factors} unique sets of factors, each with \Sexpr{replicates} -- with settings given in \cref{tbl:factor_levels_ubmech} under fixed conditions, of which we are exclusively concerned with the subset of \Sexpr{nrow(df_completed)} runs that reached completion at \Sexpr{max(df_completed['gen'])} generations.
