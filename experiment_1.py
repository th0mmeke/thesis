...
def my_get_offspring(x):
    ...
    if model.get_random_boolean(model.P_REPRODUCE, x.fitness):
        return [model.Element(model.derive(x.fitness, x.correlation), model.derive(x.correlation, x.correlation)) for i in range(random.randint(*model.N_OFFSPRING))]
    else:
        return []
...
for repeat in range(0,30):
    model.run(init_population(1000), 10)
