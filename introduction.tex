...
\begin{table}
	...
	\begin{center}
		\begin{tabular}{@{}p{5cm}p{8cm}@{}}
			...
			Conferences - major conferences not included in ACM and IEEE databases & Main publication mechanism in Computer Science\\
			PhD and MSc Thesis repositories & Previously identified works \parencite{Knibbe:2006vn}\\
			\bottomrule
		\end{tabular}
	\end{center}
...
No journals were directly included as \textit{Artificial Life} and \textit{Adaptive Behavior}, the two principal journals in the Artificial Life field \cite{Aicardi2010}, are indexed by \textit{Web of Science}.
No general internet search was performed. 
...
\begin{table}
	...
	\begin{center}
		\begin{tabular}{@{}p{10cm}r@{}}
			...
			Keywords & Count\\
			\midrule
			Artificial AND (Embryogeny OR Embryology OR Ontogeny) & 11\\
			Computational AND (Embryogeny OR Embryology OR Ontogeny) & 2\\
			Developmental & 321\\
			Embryogeny & 6\\
			``Evolutionary Development'' & 23\\
			EvoDevo & 0\\
			Generative AND (Encoding OR Mapping OR Representation) AND NOT (``Generative Topographic Mapping'' OR GTM) & 130 \\
			Indirect Encoding & 12\\
			Morphogenesis & 34\\
			\bottomrule
		\end{tabular}
	\end{center}
	\caption{Keywords used to construct search strings for the literature search, with a count of the number of matches in SpringerLink on 27 June 2012 to indicate relative volumes.}
\end{table}

Keywords chosen from research questions; alternative keywords found in review of known papers and with completion from (Stanley Gecco 2009 workshop)

Keyword "Developmental" returns many papers from Education, and from Software Engineering \eg "Developmental and operational testing \ldots"
"Networks" might refer to either computer networks (excluded) or developmental networks (included), so cannot be excluded on the basis of title alone.
``Generative'' also applies to machine learning.

\subsubsection{Stage 3 - Automated Search}

Only those papers published between 2000 and the present were included.
...
\begin{table*}
	...
	\begin{center}
		\begin{tabular}{@{}p{2cm}p{2cm}p{9cm}@{}}
			...
			Query string & Abstract:((Artificial AND (Embryogeny OR Embryology OR Ontogeny)) OR (Computational AND (Embryogeny OR Embryology OR Ontogeny)) OR Developmental OR Embryogeny OR "Evolutionary Development" OR EvoDevo OR (Generative AND (Encoding OR Mapping OR Representation)) OR "Indirect Encoding" OR Morphogenesis)\\
			\midrule
			IEEE &
			Query string & ((Artificial AND (Embryogeny OR Embryology OR Ontogeny)) OR (Computational AND (Embryogeny OR Embryology OR Ontogeny)) OR Developmental OR Embryogeny OR "Evolutionary Development" OR EvoDevo OR (Generative AND (Encoding OR Mapping OR Representation) AND NOT ("Generative Topographic Mapping" OR GTM)) OR "Indirect Encoding" OR Morphogenesis)\\\cmidrule(l){2-3}
			& Topic & Computing \& Processing (Hardware/Software), metadata-only\\
			\midrule
			Web of Science &
			Query string & TS=((Artificial AND (Embryogeny OR Embryology OR Ontogeny)) OR (Computational AND (Embryogeny OR Embryology OR Ontogeny)) OR Developmental OR Embryogeny OR "Evolutionary Development" OR EvoDevo OR (Generative AND (Encoding OR Mapping OR Representation)) OR "Indirect Encoding" OR Morphogenesis)\\\cmidrule(l){2-3}
			& Subject Areas & Computer Science\\\cmidrule(l){2-3}
			& Databases & SCI-EXPANDED, CPCI-S\\\cmidrule(l){2-3}
			& Lemmatization & On\\
			\midrule
			SpringerLink &
			Query string & ab:(Artificial AND (Embryogeny OR Embryology OR Ontogeny)) OR
			ab:(Computational AND (Embryogeny OR Embryology OR Ontogeny)) OR
			ab:(Developmental) OR
			ab:Embryogeny OR
			ab:"Evolutionary Development" OR
			ab:EvoDevo OR
			ab:(Generative AND (Encoding OR Mapping OR Representation) AND NOT ("Generative Topographic Mapping" OR GTM)) OR
			ab:"Indirect Encoding" OR
			ab:Morphogenesis\\
			\cmidrule(l){2-3}
		...
		\end{tabular}
	\end{center}
	\caption{Query parameters for search 3a. All searches restricted to the period 2000--present.}
\end{table*}
...
Of the papers reviewed in \myref{sec:biologicalmodels}, %\cite{Banzhaf:2003kx,Beslon:2010zr,Dellaert:1995po,Dellaert:1996ys,Fleischer:1994kx,Ghanem:2008ct,Jakobi:1995py,Knabe:2006vn,Knabe:2008xe,Knibbe:2006vn,Knibbe:2007kx,Kuo:2006fv,Quick:2003uq,Reil:1999rp,Rohlf:2009sh,Sanchez-Dehesa:2008uq,Turing:1952qr},
two are theses \parencite{Ghanem:2008ct,Knibbe:2006vn}, and 6 predate the study period \parencite{Dellaert:1995po,Dellaert:1996ys,Fleischer:1994kx,Jakobi:1995py,Reil:1999rp,Turing:1952qr},
leaving 9 unaccounted for \parencite{Banzhaf:2003kx,Beslon:2010zr,Knabe:2006vn,Knabe:2008xe,Knibbe:2007kx,Kuo:2006fv,Quick:2003uq,Rohlf:2009sh,Sanchez-Dehesa:2008uq}.

Three of these papers share a common research program focussed on biological robustness, but there is little otherwise in either titles or abstracts to indicate that these papers are relevant:

\cite{Knibbe:2007kx} is by the author of a previously discovered thesis; the abstract and title refer to properties of biological evolution. Only the use of Aevol (not mentioned in either) makes it relevant.

\cite{Sanchez-Dehesa:2008uq} does mention RAevol, but the other terms in the abstract are not sufficient to identify this as a relevant paper (\eg ``evolution of bacteria and their adaptation to the environment. Our model simulates the evolution of a population of artificial bacteria in a changing environment, providing us with an insight into the strategies that digital organisms develop to adapt to new conditions'')

\cite{Beslon:2010zr} cites a previously discovered paper, and shares authors and a research program (Aevol and RAevol) with that paper.

The remaining papers do however include phrases that should signal significance, although these terms were not originally included in our keywords:
\begin{itemize}
	\item \cite{Knabe:2006vn} references ``artificial genetic regulatory networks'' and ``artificial GRNs''.
	\item The title of \cite{Knabe:2008rz} includes the phrase ``Genetic Regulatory Network Models'', while the abstract again references ``artificial genetic regulatory networks''  and ``artificial GRNs''.
	\item The abstract of \cite{Banzhaf:2003kx} references ``artificial regulatory networks (ARNs)'' and ``artificial evolution''.
	\item ``Artificial genetic regulatory network model'' is included in the title of \cite{Kuo:2006fv}, while the abstract includes ``artificial regulatory network model''.
	\item The phrase ``Embodied Genetic Regulatory Network'' appears in both title and abstract of \cite{Quick:2003uq}.
	\item Finally, the title of \cite{Rohlf:2009sh} includes ``Artificial Genome Model'' while the abstract references both ``artificial genomes'' and \cite{Reil:1999rp}.
\end{itemize}
...
\begin{table*}
	...
	\begin{center}
		\begin{tabular}{@{}p{2cm}p{2cm}p{9cm}@{}}
			...
			\midrule
			ACM Guide to Computing Literature &
			Query string & Abstract:((artificial or embodied) and "regulatory network")\\
			\midrule
			IEEE &
			Query string & Abstract:((artificial or embodied) and "regulatory network")\\
			\midrule
			Web of Science &
			Query string & TS=((artificial OR embodied) AND "regulatory network")\\
			\midrule
			SpringerLink &
			Query string & ab:((artificial or embodied) and "regulatory network")\\
			\bottomrule
		\end{tabular}
	\end{center}
	\caption{Query strings for search 3b. All other parameters as in search 3a. All searches restricted to the period 2000--present.}
\end{table*}

This additional search does detect the previously discovered papers: \cite{Banzhaf:2003kx}, \cite{Quick:2003uq}, \cite{Kuo:2006fv}, \cite{Rohlf:2009sh}, and versions of \cite{Knabe:2006vn,Knabe:2008rz} \parencite{Knabe:2008vz}, and \cite{Sanchez-Dehesa:2008uq} \parencite{Sanchez-Dehesa:2007nu}.
...
%\centering\includegraphics[width=\linewidth]{Figures/SystematicMap}\\
%\end{minipage}
%\centering\caption{Stages of the Systematic Map, showing papers remaining at the conclusion of each stage.}
%\label{fig:mapstages}
%\end{figure}
...
\begin{table}
	...
	\begin{center}
		\begin{tabular}{@{}lllll@{}}
			\toprule
			Source & Stage 3a & Stage 3b & Stage 4 & Stage 5\\
			\midrule
			ACM Guide to Computing Literature & 402 & 256 & 64 & 58\\
			IEEE & 1,328 & 1 & 90 & 64\\
			Web of Science & 2,346 & 104 & 312 & 210\\
			SpringerLink & 537 & 12 & 86 & 45\\
			\bottomrule
		\end{tabular}
	\end{center}
	\caption{Papers remaining at conclusion of Stages 3-5, by data source. Dates for searches - all part b searches on 31 August 2012, part a as follows: 
		ACM: 5 July 2012, IEEE: 28 June 2012, Web of Science: 27 June 2012, SpringerLink: 28 June 2012. Only title returned for IEEE search was a Notice of Violation}
\end{table}

At conclusion of Stage 5 - selection on abstracts - there are 377 papers.
...
\begin{table}
	...
	\begin{center}
		\begin{tabular}{@{}llp{7cm}@{}}
			...
			\midrule
			Method & 110 & Investigation of developmental methods themselves, perhaps with experimental evalution in a particular application. The focus however is on the method itself.\\
			Application & 77 & Use of developmental methods to solve some particular problem, where the emphasis is on the resolution of the problem rather than on the particulars of the method.\\
			Properties & 63 & Examination of the benefits or results of developmental methods in general.\\
			Evaluation & 5 & Assessment of another method.\\
			\bottomrule
		\end{tabular}
	\end{center}
	\caption{Classes of paper, with count of papers by class}
	\label{tbl:paperclasses}
\end{table}

Keywords: manual search through abstracts. Automated methods (tag clouds, phrase finders) return too many poor results to be useful - keywords are very context dependent.
...
\newthought{RQ1: What are the main research topics for Evolutionary Development and how have these changed over time?}
...
\newthought{RQ2: What are the main publication sources?}

Primary publication sources are collated in \myref{tbl:publicationsources}.
...
%Adaptive Behavior - published by Sage, journal of the ISAB, editor Ezequiel Di Paolo.

\newthought{RQ3: What forms of research are published?}

The main classes of research are collected in \myref{tbl:paperclasses}.

\subsection{Limitations}
Subjective assessment of papers from title and abstract - may miss some relevant studies based solely on quality of
abstract.
Only published papers - missing commercial and open source works that have not been subject of published research.
...
\begin{sidewaystable}
	\begin{center}
		...
		\begin{tabular}{@{}llll@{}}
			\hline\noalign{\smallskip}
			\Gls{achem}                                                     & Type 	&& Inheritance mechanism\\ 
			\\ \noalign{\smallskip}
			\hline
			\noalign{\smallskip}
			...
			\cite{Fernando:2008xy,Fernando:2007pf}                          &&&Non-template: Lipid aggregates\\
			\hline
		\end{tabular}
	\end{center}
	\caption{Previous work evaluated against three criteria: type of model, emergence, and the form of any inheritance mechanism.}
\end{sidewaystable}
