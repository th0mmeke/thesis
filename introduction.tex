...
Goal is informational replicator (replicator with high variation potential). Unbounded evolution requires informational replicator; informational replicator is by definition capable of self-evolution.

Starting point is low-level elements plus emergence plus self-evolution.

Problem is how to get from starting point to goal? Goal is an informational replicator that has emerged from lower-level elements.
...
The informational replicator systems reviewed in this \namecref{previous-work} take different pathways towards full replication, but none fully satisfy all conditions. Instead we have  
...
Regular pertubations are also at the core of \cite{Fernando:2007pf} where ``avalanches'' of novel side-reactions are periodically triggered by the simulation. These avalanches result in new and rare reaction products which inject variation into the simulation; either by making new reactions possible, or shifting the balance between competing reactions, or by perhaps altering the reaction rates through changes to catalysis.

ODE modeling of well-mixed/continously stirred reactor in \cite{Stadler1993} was used to derive the Replicator equation by examining the outcome of various models including elements such as complementary replication and mutation. Pertubation purely through modeling of mutation; internal rather than external factor. Because ODE based, cannot be applied to fully-constructive chemistry - "finiteness [of reactions] is required" \cite[p6]{Stadler1993}.
