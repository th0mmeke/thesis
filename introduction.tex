...
Motivation: demonstrate capability for unbounded evolution in AChem

Unbounded evolution requires informational replicator
Evolution requires inheritance, variation, selection
Simple in biology; harder in the abstract
For unbounded, variation potential must be very high - must be able to represent large number of states without losing essence [Szathmary]
Inheritance and variation implies replication; selection implies sorting
Goal is informational replicator (replicator with high variation potential)
Starting point is low-level elements plus emergence
Emergence required [???]
Some set of low-level elements capable of emergence; capable of replicators; capable of informational replicators

Problem is how to get from starting point to goal? Want an informational replicator that has emerged from lower-level elements.


Informational replicator hard to design in any given AChem
Previous work has either:
handbuilt replicators (so limited applicability) e.g., Squirm3, or
provided specific, non-evolvable support in base AChem ruleset (e.g., copy operator in Avida)

No generic approach known in literature.

Motivation requires that informational replicators can be created in an AChem.
Reactions -> .... -> informational replicators
Experiments (and OOL) show straightforward pathway from reactions -> composomes (or compositional genomes [Segre] or variable replicators [Szathmary])
But significant gap from composomes to informational replicators
major differences:
number of states in phenotype that can be represented (and causally connected to inheritance)
separate genotype/phenotype - genotype -> development -> phenotype <- selection <- environment. -> .... -> informational replicators


Problem: begs question - if can go from composomes to informational replicator, then qualitative not just quantitative change - which is the reason why we want an informational replicator...So still a gap. Show progress on possible path, not necessary path. Unless can show can progress from 10 or so states with composomes, interesting but not relevant to unbounded evolution. As p16[Meijenfeldt2013]:

How does the system become evolvable in an open-ended manner? Obviously, a way of introducing and accumulating novel mutations must be implemented. Modelling has hardly been concerned with the question...



%This is not a work about that evolution. Nor does this thesis directly address the artificial equivalent of that explosion through evolution by natural selection, the subject of creative \gls{oe}--a topic that has had steady attention, without notable success\footnote{See recent review in \cite{BanzhafBaumgaertnerBeslonEtAl2016}}.

\section{Background and context}\label{background-and-context}

\subsection{The origins of life}
...
RNA hard [Mijenfeldt2013] so de novo RNA world "considered a myth by some" (all of which are not implausible in any semi-realistic AChem):

thermodynamics to create RNA very specific
unstable under all but a few conditions
relies on supply of nucleotides; unstable without
...
As has been said before, ``We take it as given that biology instantiates ENS'' \parencite{Watson2012}, but that doesn't mean that the algorithm of biology is in fact \gls{ens}. Adaptation in biology appears to precede Natural Selection, reinforcing that adaptation is possible without \gls{ens} \parencite{Watson2010}, and other forms of evolution altogether have been proposed in artificial systems (\eg the compositional evolution of \textcite{Arthur2009}), and on the boundaries of \gls{ens} in the domain of living systems (\eg composomal inheritance and \gls{hgt}). These variants are examined later, but first we concentrate on canonical \gls{ens}.
...
(composome OR "compositional genome" OR "compositional inheritance" OR "variable replicator") AND (("selection pressure" OR environment*) NEAR (variation OR varying OR changing))

Autocatalytic sets/composomes: Extending states: Polymer-GARD (Shenhav et al 2005), Exchange Polymer-GARD (Shenhav et al 2007), Universe-GARD (Markovitch et al., 2012) explicitly addresses this.

Reaction networks: Farmer et al 1986 in Vasas 2012 (compartmentalisation of networks) shows "first and only" [Meijen..., p17] demonstration of accumulating mutations in chemical networks. Polymers rather than monomers in GARD.

Stadler1993 ODE modeling of well-mixed/continously stirred reactor deriving Replicator equation. Approach is to examine outcome of various models including elements such as complementary replication and mutation. Pertubation purely through modeling of mutation; internal rather than external factor. Because ODE based, cannot be applied to fully-constructive chemistry - "finiteness is required" (p6)

Fontana1994a applied pertubations to well-mixed reaction vessel with objects taken from lambda-calculus. Organisations (entities) in all cases were observed to be self-maintaining but not self-reproducing (excluding cases that included a replication expression.) No locality or spatial effects; pertubations take the form of adding a small number of copies (10 or 50) of a random object every 30,000 or 50,000 collisions. Purpose of pertubations to assess sensitivity of model to change, rather than as a mechanism to induce change as in our case. 

\section{Approach}\label{methods}

In this \namecref{methods} we explain the approach taken in the remainder of this work: a summary of the motivating research questions, and a description of the specific methods to be used.
