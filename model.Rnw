...
Overall this \namecref{ch:models-of-inheritance} provides support for our initial working hypothesis that an effective inheritance mechanism can not only emerge from a low-heredity environment through evolution, but that it can be tuned and optimised by evolution.
