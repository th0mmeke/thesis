...
Our hypothesis is that a theoretical replicator (rather than a specific actual one, which may be constrained by its structure or evolutionary history), is not fixed \emph{ideal} or fixed \emph{real}, but instead somewhere along a degree determined by selective pressure.

Therefore we have the following goal for the work in this chapter:

\begin{enumerate}
	\item Can we describe and model a relationship between heritability and selective pressure for a theoretical replicator?
	\item 
\end{enumerate}
