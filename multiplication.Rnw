...
Each replicate runs for 100,000 iterations. The initial population for each replicate contains 10$\times$[H][H] molecules, 10$\times$FO, 20$\times$O, 10$\times$[O-][N+](=O)[N+]([O-])=O, 10$\times$N(=O)[O], and 20$\times$O=C=O (all in SMILES notation), and the initial reactor kinetic energy is set to 100 units.
