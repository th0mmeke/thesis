...
The factors and levels along with their mapping to model parameters are given in \cref{tbl:factor-levels}.

\begin{table}
	\caption{Factors mapped to model parameters, plus factor levels}\label{tbl:factor-levels}
