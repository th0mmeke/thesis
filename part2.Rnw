...
The first test therefore is to examine if inheritance emerges from low-fidelity and low-fitness initial conditions, and then test whether the population variance for inheritance decreases as predicted.
...
\emph{Approximate entropy} \todo{discussion}

\emph{Sample entropy} \todo{discussion}
...
GP-mappings and development (as in \textcite{Gaucherel2012})
