...
<<echo=FALSE>>=
a <- with(df,anova(lm(ave_cor~p_reproduce+p_selection+n_offspring+distribution+correlation_correlation)))
b <- with(df,anova(lm(ave_fit~p_reproduce+p_selection+n_offspring+distribution+correlation_correlation)))
@
...
<<echo=FALSE>>=
df<-subset(df,correlation_correlation=="Correlated" & p_selection == "Fitness-related" & p_reproduce == "Fitness-related")
