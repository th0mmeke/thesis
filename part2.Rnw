...
\section{Is the \gls{sd} of final fidelity under changing conditions greater than that under stable conditions?}
\section{Does \gls{sd} vary in proportion to the variability in the environment?}
...
ap <- ggplot(subset(results,gen==500),aes(ar_sd,ave_fid)) + geom_point() + labs(x='sd parameter to environmental model', y='Final mean fidelity')
