...
\section{Model of variation and inheritance}\label{base-model}

We now introduce a general model of the relationship between variation and inheritance in a population of evolving abstract entities (\cref{base-model-algorithm}) where the key elements of the hypothesis are represented as explicit parameters. The model is based on Model 6 of \textcite{Bourrat2015}, but simplified and formalized. Bourrat summarizes this endogenous strategy as ``explaining variables which have previously been taken for granted in a model (such as reproduction and inheritance), by reference to other, more fundamental variables present in the model.'' \parencite[p.129]{Bourrat2015}.

The model consists of a population, consisting of abstract entities, and two simple functions--\emph{Selection} and \emph{Variation}--that each take a population as input and output a transformed population. Each \gls{run} consists of a fixed number of time steps (generations), where at each step these two functions are applied in sequence to the current population to form a replacement population as documented in \cref{base-model-algorithm}. 
...
\item The new population created by \emph{Variation} consists of a number of new entities (``children'') for each entity (``parent'') in the current population (see \cref{model-functions}). Each entity has children with probability $p_{reproduction}$, and if it does, the number of children it has is some random number between $0$ and $n_{children}$. The properties of each child are related to the properties of its parent by a mapping, represented in the algorithm by the function $Derive$, which maps the parent value to a range given by a distribution of some range (the value of the function $Range$ in the algorithm) and an expected value equal to the parent's value (see \cref{fig:correlation}).
\end{enumerate}

The model is parameterized to make it easy to describe different specific models within this general structure; these parameters are defined in \cref{tbl:parameter_definitions}. Later in the chapters describing the series of experiments that use this model, these parameters will be mapped to experiment factors, with specific parameter values becoming factor levels.
...
\begin{table}
	\begin{center}
		...
		\begin{tabular}{@{}llp{8cm}@{}}
			...
			\midrule
			$p_{reproduction}$ & $0$                                  & Probability of reproduction\\
			$p_{selection}$    & $0$                                  & Probability of selection\\
			...
			Range			   & $entity\mapsto[0,1]$				  & Some function that generates a range value for Derive perhaps using an entity's fidelity or fitness values, or perhaps returning a constant value\\
			\bottomrule
		\end{tabular}
	\end{center}
\end{table}
		...
		\For{each $entity$ with $fitness$ and $fidelity$ in $population$}{
			...
			\Prob($p_{reproduction}$:){
				\For{some number of children $\in \mathcal{U}[0,n_{children}]$} {
					$a\leftarrow$ Derive($fitness$, $fidelity$)\;
					$b\leftarrow$ Derive($fidelity$, Range($entity$))\;
					Create new $child$ with fitness $a$ and fidelity $b$\;
...
\chapter{Practical experimental considerations}

In the absence of any restrictions on population size there is nothing to prevent a population growing beyond the capacity of the simulation system. This is an practical difficulty in experiment design rather than a property of the theoretical model. 
...
If \emph{Correlate Fidelity} is true, $range$ = parent's fidelity, or if false, $range$ = some random value $x\in \mathcal{U}[0,1]$.
...
\chapter{Reducing the parameter space for the experiments}
...
Before each run we construct an initial population. Entities begin with an initial fidelity and fitness chosen randomly from a uniform distribution over a specific range (see \cref{tbl:ic}) so as not to introduce any bias that might result from a repeated starting point. From the hypothesis \cref{hypothesis-2}, and driven by the overall goal of this thesis, we expect to see a population of low fidelity entities eventually replaced by one of high (or at least higher) fidelity ones. This is analogous to a key step on the path taken in biology from the abiotic world, where early copy mechanisms lacked the capabilities for high-fidelity copying.

\begin{table}[t]
	\begin{center}
		\caption{Initial conditions for each experiment run}\label{tbl:ic}
		\begin{tabular}{@{}ll@{}}
			\toprule
...
Runs in which the population dies out are incompatible with ongoing evolution, and so should be separated from other, sustainable, runs in the analysis. This situation does arise in our simulation: some runs don't reach the target of \Sexpr{max(df['gen'])} generations (\cref{fig:popoverall}), and of those that did, some did not reach the target with a sustainable population size.
...
Of the \Sexpr{nrow(subset(df_full,gen==0))} experiment runs, \Sexpr{nrow(subset(df_full,gen==0))-nrow(subset(df_full, gen==500))} fail to reach completion, and \Sexpr{english(nrow(subset(df_full, gen==500 & pop < 4000)))} complete with a population size substantially smaller ($\leq$ \Sexpr{max(subset(df_full, gen==500 & pop < 4000)['pop'])}) than the population size in the other completed runs. In fact, all other runs reach completion with a population size at the upper bound limit of \Sexpr{min(subset(df_full, gen==500 & pop >=4000)['pop'])}.
...
Note that these results also provide support for the experiment duration of \Sexpr{max(df_full['gen'])} generations - either a run maintains a stable population (and so the duration is unrelated to population size considerations), or if a population is to go extinct it does so by \Sexpr{max(df_full['gen'])} generations or soon after and so can easily be identified for special treatment.
