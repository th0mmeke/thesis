...
\begin{table}
	\begin{center}
		...
		\begin{tabular}{@{}llp{8cm}@{}}
			...
			Range		   		& $entity\mapsto\mathbb{R}$				& Some function that generates a range value for Derive (perhaps using an entity's fidelity or fitness value, or perhaps returning a constant value)\\
			\bottomrule
		\end{tabular}
	\end{center}
\end{table}
