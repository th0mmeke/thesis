...
Although several parameters in the model are continuous, at this stage the main ones can be reduced to a set of A/B alternatives, or two-level factors. Of the others, as discussed in \cref{upper-size-bound}, \emph{Population Restriction} is held to Fitness-independent sampling. $P_{reproduction}$ and $p_{selection}$ take four levels each to cover a reasonable range given the usual sensitivity of evolutionary models to these type of parameter, with three absolute values (0.33, 0.66 and 1.0) irrespective of fitness, and one meaning ``use the parent's value for this parameter.'' 
