...
From the hypothesis we make these two predictions:
\begin{enumerate}
	...
	\item The higher the variability in the environment, the higher the \gls{sd} of \emph{Fidelity}. As a corollary, the \gls{sd} of final fidelity under changing conditions will be greater than that under stable conditions.
\end{enumerate}
...
\section{Is fidelity proportional to the predictability of the environment?}
...
\section{Does the standard deviation of fidelity vary in proportion to the variability in the environment?}
...
An \gls{anova} model evaluating a linear relationship between the standard deviation of fidelity, and the parameters $\delta$, $\sigma$ and $\Theta$ to the environmental model suggests that all three parameters have some effect on the \gls{sd} of fidelity, with the influence of $\delta$ and $\Theta$ being highly significant (p\textless 0.001) and that of $\sigma$ significant (p\textless 0.01). 
...
%Visual inspection of \cref{fig:sd-fidelity} provides some support for this, with the standard deviation clearly decreasing as the value of $\delta$ approaches zero from below.
...
The corollary of this prediction is that the \gls{sd} of fidelity will be highest in the least predictable environments, and lowest in the most stable (when all three parameters are near 0).

%<<echo=FALSE>>=
%mean(subset(temp,abs(theta)<0.1 & abs(delta)<0.01 & abs(sigma)<0.1)[,'sd_fidelity'])
%mean(subset(temp,abs(theta)>=0.1 & abs(delta)>=0.01 & abs(sigma)>=0.1)[,'sd_fidelity'])
%@
