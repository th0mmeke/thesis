...
The bound is implemented by a fitness-independent sampling of $n$ entities from the population (as given in the $Restriction$ function in \cref{upper-bound-model-algorithm}), if and only if the pre-sampling population size is greater than $n$.

\begin{algorithm}
	\Def{Restriction(population,n)}{
		\Return $\text{random sample of }n\text{ entities from }population$\;
	}
...
\begin{algorithm}
	\Def{Restriction(population,n)}{
		...
		\Return $\text{first }n\text{ entities from }sortedPopulation$\;
	}
