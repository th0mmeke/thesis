...
First we define the two distinct types of relationship between an entity and its environment mentioned in the previous \namecref{methods-p2}: stable, and changing environments. A stable environment is one where the relative fitness of each entity with respect to the environment does not change over time. An changing environment is the other case, where the relative fitness does change over time. These definitions of course assume that the phenotype of the entity remains constant over its lifespan, and hence that the only cause of a fitness change can be a change in environment rather than, for example, the entity learning or developing, which are both processes that change the phenotype on a shorter timescale than a lifespan.
...
The parameter $n_{offspring}$ controls two related aspects of the model. It directly determines the maximum number of new entities derived from an existing entity at each generation, and indirectly it affects the stablity of the population as it strongly influences the proportion of new versus old entities. Finally, when either $p_{reproduction}$ or $p_{selection}$ is absolute rather than relative, we would also expect the population size to be highly sensitive to $n_{offspring}$ as the numbers of new entities may not be sufficient to sustain a population undergoing, for example, a continuous cull.
...
Finally, from the discussion around the effect of $p_{reproduction}$, these factors and levels can be clustered into only two intrarelated groups: group one where $p_{reproduction}$ takes the level ``$fitness$'' and group two where it takes any of the remaining, absolute, levels.
...
\subsection{Experimental design}
We use the same experimental design and results dataset as in the previous \namecref{does-average-inheritance-approach-perfect-inheritance}.
...
The initial hypothesis (\cref{hypothesis-2}) assumes a perfectly predictable environment that can be exactly ``learned'' by an evolutionary algorithm. If we assume that learnabilty is related to predictability, then we can restate the hypothesis in terms of predictability:

\begin{hypothesis}
	Variation, where there is some initial correlation between generations for a property, and Selection are sufficient for Inheritance, where the fidelity of inheritance is proportional to the predictability of the environment.
\end{hypothesis}\label{hypothesis-3} 

From the hypothesis we make these predictions:
\begin{enumerate}
	\item Fidelity is proportional to the predictability of the environment (directly from the hypothesis.)
	\item Fidelity is at a minimum in conditions of maximum unpredictability, that is for time-series where $\Theta=0$, $\delta=0$ and $\sigma_e>0$.
	\item Fidelity is at a maximum in stable conditions, as already shown in \cref{experimental-test-of-h2-under-fixed-conditions}.
...
\begin{itemize}
	...
	\item The time-series is defined by three independent elements, two predictable (driven by $\Theta$ and $\delta$) and the other ($sigma_e$) random and unlearnable. By changing the ratio between the two we can examine the performance of the evolutionary algorithm on some continuum of predictability.
...
Finally, for an environmental model defined according to \cref{ar-1-time-series}, the combination of $\Theta$, $\sigma_e$ and $\delta$ provides one measure of \emph{environmental predictability}.
...
<<factorialthetasd, pdfcrop=TRUE, echo=FALSE, warning=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Visualisation of the fitness changes at each time interval (top facet) and cumulative fitness change (bottom facet) that result from our environmental model for bias=0 with two values of sd (left and right), and three values of theta (each facet row.)'>>=
library(reshape2)
library(ggplot2)
library(cowplot) # styling of plots, extension of ggplot2
library(gridExtra)
...
<<factorialsdbias, pdfcrop=TRUE, echo=FALSE, warning=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Visualisation of the fitness changes at each time interval (top facet) and cumulative fitness change (bottom facet) for theta=0 with three values of bias (columns), and two values of sd (each facet row.)'>>=
...
<<sampleenvironment, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Example time-series produced by our environmental model for a sample of theta, sd and bias values. These are incremental fitness changes, rather than cumulative ones.'>>=
library(reshape2)
library(ggplot2)
library(cowplot) # styling of plots, extension of ggplot2
...
<<cumulativefitness, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, fig.pos='htp', fig.cap='Cumulative time-series examples for samples of theta, sd and bias.'>>=
for (r in unique(t2$run)) {
	t2[t2$run==r,'t'] <- 1:50 # tag with timestamp
...
<<figure22, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, fig.pos='htp', fig.cap='Distribution of maximum generation reached for all runs of dataset no.1 (top), and dataset no.2 (bottom). Axes are to the same scale. Runs (along the x-axis) are ordered by final generation reached.'>>=
library(ggplot2)
library(gridExtra)
library(cowplot) # styling of plots, extension of ggplot2
...
A summary of the results from each of the four datasets is given in \cref{tbl:summary-for-changing-datasets}. The difference in the proportion of completed runs to total runs reflects the respective ranges of the independent variables (as seen in \cref{tbl:range-of-independent-variables}), shown graphically in \cref{fig:figure22}.
...
<<figure23, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, fig.pos='htp', fig.cap='Final mean fidelity against sd parameter to environmental model (top) and final standard deviation of fidelity against bias parameter (bottom), for all experimental runs.'>>=
library(ggplot2)
library(gridExtra)
library(cowplot) # styling of plots, extension of ggplot2

ap <- ggplot(subset(results,gen==500),aes(ar_sd,ave_fid)) + geom_point() + geom_smooth(method='lm') + labs(x='sd parameter to environmental model', y='Final mean fidelity')
bp <- ggplot(subset(results,gen==500)) + geom_point(aes(ar_bias,ave_fid)) + labs(x='bias parameter to environmental model', y='Final mean fidelity') # fid ~ -bias when bias <0, otherwise fid can reach max
grid.arrange(ap,bp,nrow=2,ncol=1)
...
@
<<figure24, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE,fig.pos='htp', fig.cap='Subset of results where $0.002 < \\lvert bias\\rvert$, again showing final mean fidelity (top) and final standard deviation of fidelity (bottom) against sd parameter.'>>=
library(ggplot2)
library(gridExtra)
library(cowplot) # styling of plots, extension of ggplot2

ap <- ggplot(subset(results,gen==500 & -0.002<ar_bias & ar_bias<0.002),aes(ar_sd,ave_fid)) + geom_point()  + geom_smooth(method='lm') + labs(x='sd parameter to environmental model', y='Final mean fidelity') # ave fid drops as sd increases
bp <- ggplot(subset(results,gen==500 & -0.002<ar_bias & ar_bias<0.002),aes(ar_sd,sd_fid)) + geom_point()  + geom_smooth(method='lm') + labs(x='sd parameter to environmental model', y='Final standard deviation of fidelity') # sd fid increases as sd increases
grid.arrange(ap,bp,nrow=2,ncol=1)
@
...
<<figure26, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE,fig.pos='htp', fig.cap='Subset of results where $0.002 < \\lvert bias\\rvert$, showing the effect of lineage. Final mean fidelity (top) and final standard deviation of fidelity (bottom) against sd parameter. Very little difference is observed between applying an environmental change uniformly across all entities, or applying different changes to each lineage.'>>=
library(ggplot2)
library(gridExtra)
library(cowplot) # styling of plots, extension of ggplot2

ap <- ggplot(subset(results,gen==500 & -0.002<ar_bias & ar_bias<0.002),aes(ar_sd,ave_fid)) + geom_point()  + geom_smooth(method='lm')  + facet_wrap(~BY_LINEAGE)+ labs(x='sd parameter to environmental model', y='Final mean fidelity')
bp <- ggplot(subset(results,gen==500 & -0.002<ar_bias & ar_bias<0.002),aes(ar_sd,sd_fid)) + geom_point()  + geom_smooth(method='lm')  + facet_wrap(~BY_LINEAGE) + labs(x='sd parameter to environmental model', y='Final standard deviation of fidelity')
grid.arrange(ap,bp,nrow=2,ncol=1)
@
