...
\begin{table} % d9f18e2
	\begin{center}
		...
		\begin{tabular}{@{}llp{6cm}@{}}
			...
			Distribution           & 2                & normal dist., $\mathbb{N}$ or uniform dist., $\mathcal{U}$ \\
...
The $Range$ parameter deserves further discussion as it is fundamental to the model. The value produced by $Range$ determines the degree to which $Derive$ varies from the given expected value; $Range$ therefore describes the degree of correlation between the expected value input and the output of $Derive$. A small $Range$ value will produce a high degree of correlation between input and output. 

Now if $Range$ is based on an entity's fidelity value, for example if $Range =\lvert 1-fidelity\rvert$, then high values of fidelity will result in a small range and therefore a high degree of correlation between the input and output of $Derive$; low fidelity values will see $Range$ being large, and $Derive$ will have a low degree of correlation. This corresponds to our intuitive sense of how inheritance should work. Some mechanisms will produce terrible copies--fidelity is low and $Derive$ is poorly correlated. Others are perfect replicators--fidelity is perfect (1.0), and $Derive$ will be perfectly correlated. On the other hand, if $Range$ is given a value unrelated to the entity's fidelity value, the output of $Derive$ is uncorrelated with fidelity; this matches our intuitive sense of a non-functioning inheritance mechanism.

Why do we not just remove the parameter $Range$ and define the range of $Derive$ as $\lvert 1-fidelity\rvert$? First, to some degree this would beg the question of this model; we will have pre-determined that inheritance is a mechanism based on fidelity. More interestingly though, it would also remove our ability to explore the effect of different models for $Range$, as we do in a limited way in following sections.

The factors and levels used for are given in \cref{tbl:factor-levels}.

\begin{table}
	\caption{Factor levels for experiments to determine parameter settings}\label{tbl:factor-levels}
	\begin{tabular}{@{}llp{3cm}p{9cm}@{}}
		\toprule
		...
		Reproduction		   &Reproduction		&2				 	& Correlated and Uncorrelated (see \cref{})\\
		Distribution           &--					&2                	& normal dist., $\mathbb{N}$ or uniform dist., $\mathcal{U}$\\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{tabular}{@{}lll@{}}
\toprule
&Normal dist., $\mathbb{N}$	&uniform dist., $\mathcal{U}$\\
\midrule
Correlated		&\makecell{$fitness_{new}\leftarrow \mathbb{N}, \mu=fitness, \sigma=1-fidelity$\\$fidelity_{new}\leftarrow\mathbb{N}, \mu=fidelity, \sigma=1-fidelity$}& \makecell{$fitness_{new}\leftarrow \mathcal{U}[0, 1.0]$\\$fidelity_{new}\leftarrow \mathcal{U}[0, 1.0]$}\\
\midrule
Uncorrelated	&\makecell{$fitness_{new}\leftarrow \mathbb{N}., \mu=fitness, \sigma=1-fidelity$\\$fidelity_{new}\leftarrow\mathbb{N}, \mu=fidelity, \sigma=\mathcal{U}[0, 1.0]$}&\makecell{$fitness_{new}\leftarrow \mathcal{U}[0, 1.0]$\\$fidelity_{new}\leftarrow \mathcal{U}[0, 1.0]$}\\
\bottomrule
\end{tabular}
\end{table}
...
<<normal, pdfcrop=TRUE, echo=FALSE, cache=TRUE, out.width='0.45\\linewidth',fig.show='hold'>>=
...
\begin{enumerate}
	...
	\item The variance of inheritance in the population will decrease more than expected by chance only.
\end{enumerate}
