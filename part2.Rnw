...
<<mean-fidelity-fitness, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Smoothed ranges over time of the mean fidelity (top) and mean fitness (bottom) for all levels of all factors. Lines that drop below zero are an artifact of the line smoothing algorithm used (local polynomial regression fitting).'>>=
...
<<sd-fidelity-fitness, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Smoothed ranges (local polynomial regression fitting) for the standard deviation rather than the mean - fidelity on the top row, fitness below.'>>=
...
According to \textcite{Zliobaite:2009cr}, the main types of concept drift can be classified using two attributes--speed, and reoccurrence--where speed can be either sudden or gradual, and reoccurrence either always new or reoccurring.  
