...
Models (not simulations) for the emergence of inheritance, for the purpose of strengthening the theoretical basis for any simulation.
...
\begin{NOTES}
In biology, standard population genetics describes the dynamics over time of genotype frequencies, but remains bound to biology. The models and methods are drawn from biological theory, but the goal remains the prediction of biology population evolution. \cite{Paenke:2007ie} and \cite{Gaucherel2012} compare the inheritance of non-acquired characteristics and acquired characteristics. Synthesis paper \cite{Schuster2011} identifies error threshold rates for mutation (Eigen1971), work on fitness landscapes, and neutral landscapes (relationship between genotypes and phenotypes)
	
\cite{Paixao2015} sought common principles between population genetics and evolutionary computation in an attempt to unify the two fields. Similarly MWUAs seek a deeper meaning to evolution, placing it as one of a family of closely related algorithms all based on multiplicative weights.
...
\begin{table}
	\begin{center}
		\caption{Factor levels for investigation into inheritance}\label{tbl:factor-levels-for-investigation-into-inheritance-under-low-start-conditions}
...
\begin{NOTES}
	\cite[79]{Schuster2011} relationship between fitness landscapes and error thresholds - ``(i) the single-peak landscape corresponding to a mean field approximation, (ii) the hyperbolic landscape, (iii) the step-linear landscape, (iv) the multiplicative landscape, and (v) the additive or linear landscape.''
\end{NOTES}

The model must describe two particular elements of the environmental change: the scope of the change, and the shape of the change. First, we discuss the scope of change. Our evolutionary model allows us to group entities at three different levels:
\begin{enumerate}
