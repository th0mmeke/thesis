...
colnames(df_full)[14] <- "Correlation"

df <- subset(df_full, gen==500)
df$p_reproduce = factor(df$p_reproduce, labels=c('Fitness','0.33','0.66','1.0'))
df$p_selection = factor(df$p_selection, labels=c('Fitness','0.33','0.66','1.0'))
df$Distribution = factor(df$Distribution, labels=c('Bounded\nnormal','Sampled\nnormal','Uniform'))
df$Correlation <- factor(df$Correlation, labels=c('Uncorrelated','Correlated'))
@
...
<<echo=FALSE>>=
a <- with(df,anova(lm(ave_cor~p_reproduce+p_selection+n_offspring+Distribution+Correlation)))
b <- with(df,anova(lm(ave_fit~p_reproduce+p_selection+n_offspring+Distribution+Correlation)))
@
...
<<p-reproduction, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold',fig.cap=''>>=
...
<<p-selection, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold',fig.cap=''>>=
...
<<n-offspring, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold',fig.cap=''>>=
...
Confirmation with a single-factor \gls{anova} test with factor $n_{offspring}$ and response variables $\overline{fidelity}_{end}$ and $\overline{fitness}_{end}$ gives F=\Sexpr{a[,'F value'][1]} and p=\Sexpr{a[,'Pr(>F)'][1]}, and F=\Sexpr{b[,'F value'][1]} and p=\Sexpr{b[,'Pr(>F)'][1]}, respectively. However, a visual check of the population growth over time shows a quite different situation (\cref{fig:n-offspring-population}.)

<<n-offspring-population, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Population size over time for two values of factor $n_{children}$: $n_children=2$ (on the left) and $n_children=5$ (on the right.)'>>=
...
As might be expected, the higher reproduction rate in the right hand facet also resulted in a slightly higher proportion of runs reaching completion at \Sexpr{max(df_full['gen'])} generations: \Sexpr{nrow(subset(df,n_offspring==2))} when $n_{offspring}=2$ against \Sexpr{nrow(subset(df,n_offspring==5))} when $n_{offspring}=5$.
...
<<normal-normal-uniform, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold',fig.cap=''>>=
...
<<correlated-uncorrelated, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Comparison between correlated and uncorrelated values for the factor \\textbf{Reproduction} showing results for final mean fidelity ($\\overline{fidelity}_{end}$, on the left) and final mean fitness ($\\overline{fitness}_{end}$, on right)'>>=
ap <- ggplot(df) + geom_boxplot(aes(x=Correlation,y=ave_cor)) + labs(x='',y='Final mean fidelity')
bp <- ggplot(df) + geom_boxplot(aes(x=Correlation,y=ave_fit)) + labs(x='',y='Final mean fitness')
grid.arrange(ap,bp,nrow=1,ncol=2)
@
...
<<echo=FALSE>>=
df<-subset(df,Correlation=="Correlated" & p_selection == "Fitness-related" & p_reproduce == "Fitness-related")
a <- with(df,anova(lm(ave_cor~n_offspring+Distribution)))
b <- with(df,anova(lm(ave_fit~n_offspring+Distribution)))
@
<<results='asis',echo=FALSE>>=
print.xtable.booktabs(a)
@
