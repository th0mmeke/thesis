...
<<p-reproduction, pdfcrop=TRUE, echo=FALSE, cache=TRUE, out.width='0.45\\linewidth',fig.show='hold',fig.cap=''>>=
...
<<p-selection, pdfcrop=TRUE, echo=FALSE, cache=TRUE, out.width='0.45\\linewidth',fig.show='hold',fig.cap=''>>=
ap <- ggplot(df) + geom_boxplot(aes(x=selection,y=ave_cor)) + labs(x='',y='Final mean fidelity')
bp <- ggplot(df) + geom_boxplot(aes(x=selection,y=ave_fit)) + labs(x='',y='Final mean fitness')
grid.arrange(ap,bp,nrow=1,ncol=2)
@

\section[Sensitivity to number of offspring]{Sensitivity to the parameter $n_{offspring}$}

<<n-offspring, pdfcrop=TRUE, echo=FALSE, cache=TRUE, out.width='0.45\\linewidth',fig.show='hold',fig.cap=''>>=
...
Confirmation with a single-factor \gls{anova} test with factor $n_offspring$ and response variables $\overline{fidelity}_{end}$ and $\overline{fitness}_{end}$ gives F=\Sexpr{a[,'F value'][1]} and p=\Sexpr{a[,'Pr(>F)'][1]}, and F=\Sexpr{b[,'F value'][1]} and p=\Sexpr{b[,'Pr(>F)'][1]}, respectively. However, a visual check of the population growth over time shows a quite different situation (\cref{fig:n-offspring-population}.)

<<n-offspring-population, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Population size over time for two values of parameter $n_{children}$ - 2 (on the left) and 5 (on the right) .'>>=
...
As might be expected, the higher reproduction rate in the right hand facet also resulted in a slightly higher proportion of runs reaching completion at \Sexpr{max(df_full['gen'])} generations: when $n_offspring=2$, \Sexpr{nrow(subset(df,n_offspring==2))} against \Sexpr{nrow(subset(df,n_offspring==5))} when $n_offspring=5$, 
...
<<normal-normal-uniform, pdfcrop=TRUE, echo=FALSE, cache=TRUE, out.width='0.45\\linewidth',fig.show='hold',fig.cap=''>>=
