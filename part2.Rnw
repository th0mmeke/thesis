...
Addressing these limitations becomes the driver for the work in this \namecref{part2}. The method follows that outlined in \cref{approach}: 

\begin{enumerate}
	\item Construction of a parameterised simulation model [\cref{base-model}]
	\item Forming a hypothesis [\cref{h2}]
	\item Characterising the sensitivity of the simulation model to parameter ranges [\cref{reducing-the-parameter-space-for-the-experiments}]
...
First we define the two distinct types of relationship between an entity and its environment--stable, and changing--mentioned in an earlier \namecref{methods-p2}. A stable environment is one where the relative fitness of each entity with respect to the environment does not change over time, whereas it does in a changing environment. These definitions of course assume that the phenotype of the entity remains constant over its lifespan, and hence that the only cause of a fitness change can be a change in environment rather than, for example, the entity learning or developing. These are both processes that change the phenotype on a shorter timescale than a lifespan.
...
Similar considerations apply to our model. Because we observe that the population size increases exponentially in many experimental runs (\eg right-hand side of \cref{fig:unboundedplot}), some restriction on population size is needed, and it's important that we eliminate the possibility of introducing bias to the results from the mechanism used to control population size.
...
\section{Experimental design}\label{experimental-design-c5}
<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df <- subset(load.results('results/results-819350e-unrestricted.data'), ecf==0)
@

All experiments in this \namecref{practical-considerations} are based on the following factorial experiment design, with factors and factor levels given in \cref{tbl:factor-levels-c5}. The environment is considered ``stable'' (that is, each element's fitness value does not change); the initial population consists of \Sexpr{df[1,]['pop']} elements, and each run is for a maximum of \Sexpr{max(df['gen'])} generations.
...
\section[Choice of limit mechanism]{Is the choice of limit mechanism significant?}
...
With this initial experiment design without an upper population bound, no run continued for more than a handful of generations before the population size reached more than ten times the initial size. It is clear from \cref{fig:unboundedplot} that some form of upper bound is necessary. Our implementation introduces a \emph{Restriction} function, mapping $population$ to a restricted $population$, into the base model algorithm from \cref{base-model}. The new algorithm is shown in \cref{upper-bound-model-algorithm}. 

This section describes two possible approaches to the implementation of the \emph{Restriction} function and attempts to understand the form of the bias each introduces into the experimental results.
...
\subsection{Fitness-independent sampling}\label{fitness-independent-sampling}
...
\subsection{Fitness-based selection}
...
It also recognises that in both Evolutionary Computation and natural populations, the population limit is a function of the carrying capacity of the environment and fitness determines the selection.
...
The design is the same as in \cref{experimental-design-c5} with the addition of a further factor for the restriction function, taking two levels: fitness-independent or fitness-based.
...
As our interest is in the sensitivity of the model to each parameter, a factorial design (as described in \cref{approach}) is preferred with factors corresponding to the parameters, and dependent or response variables, determined by the hypothesis predictions, of mean population fidelity and mean population fitness.

%A full factorial design with seven 2-level factors would require testing $2^{7}$ combinations of factor values, or 128 sets of replicated runs, while a $2^{(7-3)}$ fractional factorial design \parencite{Montgomery2009}\footnote{\eg  \url{http://www.itl.nist.gov/div898/handbook/pri/section3/eqns/2to7m3.txt}} can reduce this to 16 sets of replicated runs without loss of validity on the assumption that 3-factor interactions and higher are not significant. In other words, a $2^{(7-3)}$ design is sufficient to separate the main effect from any 2-factor interactions. This seems a reasonable trade-off between discriminatory power and the total number of runs required, given our goal is to eliminate uninteresting settings for the next set of experiments. Note also that as each level of each factor occurs the same number of times in the results, the design remains ``balanced'' in the statistical sense, greatly easing analysis.
...
Based on preliminary runs, for this initial exploration we start with 3 replicates for each combination of factor values in the design. 
...
Before each run we construct an initial population. From the hypothesis \cref{hypothesis-2}, we expect to see a population of low fidelity entities eventually replaced by one of high (or at least higher) fidelity ones. Therefore our initial population consists of entities with an initial fidelity $\in\mathcal{U}[0, 0.3]$ and fitness $\in\mathcal{U}[0, 0.3]$ chosen randomly from a uniform distribution so as to reduce the influence of any particular starting point. 

<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df_full <- load.results('results/results-parameters.data')
...
df_full$p_reproduce = factor(df_full$p_reproduce, labels=c('Fitness','0.33','0.66','1.0'))
df_full$p_selection = factor(df_full$p_selection, labels=c('Fitness','0.33','0.66','1.0'))
df_full$Distribution = factor(df_full$Distribution, labels=c('Bounded\nnormal','Sampled\nnormal','Uniform'))
...
The response of the model to factor levels can quickly be assessed by a simple \gls{anova} fixed-effects model \parencite[p.64]{Montgomery2009}, with the informal null hypothesis ($H_0$) that there is no difference in the mean of the response variable between the various combinations of factor levels. With a response variable of final mean population fidelity ($\overline{fidelity}_{end}$, or the mean value for \emph{fidelity} at the end of a run) and factors and levels as given in \cref{tbl:factor-levels-c6}, the resulting \gls{anova} analysis is as follows:
	
<<echo=FALSE>>=
a <- with(df,anova(lm(ave_cor~p_reproduce+p_selection+n_offspring+Distribution+Reproduction)))
b <- with(df,anova(lm(ave_fit~p_reproduce+p_selection+n_offspring+Distribution+Reproduction)))
@
...
And for the response variable $\overline{fitness}_{end}$, the \gls{anova} analysis is:
...
This suggests that the model is highly sensitive (p\textless 0.001) to the choice of \emph{Distribution} and \emph{Reproduction} for $\overline{fidelity}_{end}$, and to all parameters except $n_{offspring}$ for $\overline{fitness}_{end}$. However, this is perhaps overly simplistic as it ignores parameter values or combinations of parameter values that are either unlikely to be of interest or unsuitable for other reasons. These nuances are explored in the following \namecrefs{sensitivity-to-p-reproduction}.

\section[Sensitivity to probability of reproduction]{Sensitivity to the parameter $p_{reproduction}$}\label{sensitivity-to-p-reproduction}
The parameter $p_{reproduction}$ in the simulation model \cref{base-model} is either related to the entity's fitness, or is set to a fixed value for all entities, irrespective of their fitness. We would expect, based on the substantial literature in \gls{ea}, that our simulation model would be highly sensitive to the value of this parameter.

<<p-reproduction, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold',fig.cap='Effect of the factor $p_{reproduction}$ on final mean fidelity ($\\overline{fidelity}_{end}$, on the left) and final mean fitness ($\\overline{fitness}_{end}$, on the right)'>>=
ap <- ggplot(df) + geom_boxplot(aes(x=p_reproduce,y=ave_cor)) + labs(x='',y='Final mean fidelity')
bp <- ggplot(df) + geom_boxplot(aes(x=p_reproduce,y=ave_fit)) + labs(x='',y='Final mean fitness')
grid.arrange(ap,bp,nrow=1,ncol=2)
@

\Cref{fig:p-reproduction} shows a visible difference in the effect on $\overline{fitness}_{end}$ between fixed values of $p_{reproduction}$ and a value equal to the entity's fitness value. Fixed $p_{reproduction}$ values have little foundation in biology, but in artificial systems it's possible to imagine a combination where either $p_{reproduction}$ or $p_{selection}$ is fixed (or absolute) while the other is based on a fitness value (where the effect instead is relative to other entities).
...
<<p-selection, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold',fig.cap='Effect of the factor $p_{selection}$ on final mean fidelity ($\\overline{fidelity}_{end}$, on the left) and final mean fitness ($\\overline{fitness}_{end}$, on the right)'>>=
ap <- ggplot(df) + geom_boxplot(aes(x=p_selection,y=ave_cor)) + labs(x='',y='Final mean fidelity')
bp <- ggplot(df) + geom_boxplot(aes(x=p_selection,y=ave_fit)) + labs(x='',y='Final mean fitness')
grid.arrange(ap,bp,nrow=1,ncol=2)
@

The relationship between $p_{selection}$ and $\overline{fidelity}_{end}$ and $\overline{fitness}_{end}$ closely that of $p_{reproduction}$; little visual difference between the effect on fidelity, but a clear difference on fitness (as would be expected for Selection.)

\section[Sensitivity to number of offspring]{Sensitivity to the parameter $n_{offspring}$}
The parameter $n_{offspring}$ controls two related aspects of the model. It directly determines the maximum number of new entities derived from an existing entity at each generation, and indirectly it affects the stability of the population as it strongly influences the proportion of new versus old entities. Finally, when either $p_{reproduction}$ or $p_{selection}$ is absolute rather than relative, we would also expect the population size to be highly sensitive to $n_{offspring}$ as the numbers of new entities may not be sufficient to sustain a population undergoing, for example, a continuous cull.

<<n-offspring, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold',fig.cap='Effect of the factor $n_{offspring}$ on final mean fidelity ($\\overline{fidelity}_{end}$, on the left) and final mean fitness ($\\overline{fitness}_{end}$, on the right)'>>=
ap <- ggplot(df) + geom_boxplot(aes(x=n_offspring,y=ave_cor)) + labs(x='',y='Final mean fidelity')
bp <- ggplot(df) + geom_boxplot(aes(x=n_offspring,y=ave_fit)) + labs(x='',y='Final mean fitness')
grid.arrange(ap,bp,nrow=1,ncol=2)
@
<<echo=FALSE>>=
a <- with(df,anova(lm(ave_cor~n_offspring)))
b <- with(df,anova(lm(ave_fit~n_offspring)))
@

From \cref{fig:n-offspring} it appears that $n_{offspring}$ is not significant in the behaviour of the simulation model, supported by a single-factor \gls{anova} test with factor $n_{offspring}$ and response variables $\overline{fidelity}_{end}$ and $\overline{fitness}_{end}$ gives F=\Sexpr{a[,'F value'][1]} and p=\Sexpr{a[,'Pr(>F)'][1]}, and F=\Sexpr{b[,'F value'][1]} and p=\Sexpr{b[,'Pr(>F)'][1]}, respectively. However, a visual check of the population growth over time shows some interesting differences between the two factor levels (\cref{fig:n-offspring-population}.) As expected, the higher reproduction rate in the right hand facet resulted in a slightly greater proportion of runs reaching completion at \Sexpr{max(df_full['gen'])} generations: \Sexpr{nrow(subset(df,n_offspring==2))}/\Sexpr{nrow(subset(df_full,n_offspring==2&gen==0))} when $n_{offspring}=2$ against \Sexpr{nrow(subset(df,n_offspring==5))}/\Sexpr{nrow(subset(df_full,n_offspring==5&gen==0))} when $n_{offspring}=5$. 

<<n-offspring-population, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Population size over time for two levels of factor $n_{children}$, $n_children=2$ (on the left) and $n_children=5$ (on the right.)'>>=
ggplot(df_full) + geom_point(aes(x=gen,y=pop), size=0.25) + labs(x="Generation", y="Population size") + facet_wrap(~n_offspring)
@

Note that the vertical lines on the extreme left of both facets of \cref{fig:n-offspring-population} are characteristics of the initial conditions: runs where $n_{offspring} = 2$ and the probability of reproduction or selection is low, either because the parameter has a low, fixed, value or because they are derived from the parent's value, suffer high selection and low reproduction initially. Taking the case where both are given by the parent's value, $p_{reproduction} = p_{selection} = $\Sexpr{mean(subset(df_full,gen==0)[,'ave_fit'])}. The difference between the two facets can be explained by the higher reproduction rate in the right-hand facet.

\section{Sensitivity to the parameter Reproduction}
The parameter $Reproduction$ describes the relationship between the properties of an entity and the properties of the related entity from which it is derived. In the simulation model this parameter is completely general, defined only as a mapping from $entity$ to $entity$. In attempting to characterise the sensitivity of the model to this parameter, we introduce two linked factors of two levels each, \emph{Reproduction} and \emph{Distribution}, that in combination map to the parameter $Reproduction$.

The factor \emph{Reproduction} from \cref{tbl:factor-levels-c6} describes two forms of relationship between parent and child entity. The first is ``correlated'' where the value of each child property has some significant relationship or correlation to the same property in the parent. A simple example of correlated properties from biology might be height: the height of a child at adulthood has a statistically significant correlation with the height of his or her parent. The second relationship is ``uncorrelated'' where the child property value is chosen independently of the parent's value. Our expectation is that the simulation model will prove sensitive to the choice of level for \emph{Reproduction} as ``uncorrelated'' levels are essentially un-evolutionary.

From \cref{correlated-uncorrelated} it is clear that there is a significant difference for both $\overline{fidelity}_{end}$ and $\overline{fitness}_{end}$ between the two levels of \emph{Reproduction}, as expected.
...
As \emph{Distribution} is used by \emph{Reproduction} to influence the relationship between the properties of an entity and its offspring, it seems plausible to hypothesise that different mapped shapes may have a significant effect on the experimental result. In \cref{fig:normal-normal-uniform} this is confirmed; the response, particularly for $\overline{fidelity}_{end}$, to the two levels for normal distributions are qualitatively different to that of a uniform distribution. In fact, the response to the uniform distribution shows that $\overline{fidelity}_{end}$ is almost completely determined by this factor level, regardless of the levels of any other factor.
...
<<example_distributions, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Example showing a normal distribution with mean = 0.5, standard deviation = 0.5 (left), the effect of folding to range $[0,1]$ of the same distribution (centre), and same distribution filtered through the sampling algorithm.'>>=
...
<<normal-normal-uniform, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.show='hold',fig.cap='Effect of the factor \\textbf{Reproduction} on final mean fidelity ($\\overline{fidelity}_{end}$, on the left) and final mean fitness ($\\overline{fitness}_{end}$, on the right)'>>=
...
<<correlated-uncorrelated, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Comparison between correlated and uncorrelated levels for the factor \\textbf{Reproduction} showing results for final mean fidelity ($\\overline{fidelity}_{end}$, on the left) and final mean fitness ($\\overline{fitness}_{end}$, on the right)'>>=
ap <- ggplot(df) + geom_boxplot(aes(x=Reproduction,y=ave_cor)) + labs(x='',y='Final mean fidelity')
bp <- ggplot(df) + geom_boxplot(aes(x=Reproduction,y=ave_fit)) + labs(x='',y='Final mean fitness')
grid.arrange(ap,bp,nrow=1,ncol=2)
@


\section{Conclusions}
For the model overall, the values of \emph{Distribution} and \emph{Reproduction} have a significant effect on $\overline{fidelity}_{end}$, while $\overline{fitness}_{end}$ is responsive to all parameter values with the exception of $n_{offspring}$. Therefore we can conclude that the model is sensitive to all parameters with the probable exception (see earlier discussion) of $n_{offspring}$, and that the response is driven by the values of \emph{Distribution} and \emph{Reproduction} and by the boundary between absolute and relative values for $p_{reproduction}$ and $p_{selection}$. This suggests that the sensitivity of the simulation model can in fact be described in simpler terms related to combinations of related values of these key drivers. As one example, models where \emph{Distribution} takes ``uniform'' form a single class of their own as the other factor levels are completely overridden by this level of this factor.

This suggests the following implications:
...
<<lowstartbyfactor, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Summary results, this time averaged across all replicates of each unique combination of factor levels (that is, grouped by factor level). Dark-coloured points represent values at end of run; light-coloured points in a horizontal line are for initial values, reflecting averaged initial conditions.'>>=
...
<<mean-fidelity-fitness, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Ranges by generation of the mean fidelity (top) and mean fitness (bottom) for all levels of all factors. Lines that drop below zero are an artifact of the line smoothing algorithm used (local polynomial regression fitting).'>>=
...
\section{Does the variance of inheritance decrease over time in the population?}\label{variance-of-inheritance-stable}
...
\subsection{Results and discussion}
<<sd-fidelity-fitness, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Ranges by generation of the standard deviation of fidelity on the top row, and standard deviation of fitness below.'>>=
...
In a similar fashion to the procedure in \cref{does-average-inheritance-approach-perfect-inheritance}, by visual inspection of \cref{sd-fidelity-fitness}, the variance (square of the standard deviation) does decrease towards zero in all cases. Once again, the runs appear to fall into two distinct groups based on the speed of convergence towards zero, and as seen in \cref{}, these runs are exclusively associated with runs where both factors $p_{reproduction}$ and $p_{selection}$ are set to $0.66$. These levels are also responsible for the two uppermost groups of final standard deviation of fitness in the lower section of \cref{sd-fidelity-fitness}. The distinction between these two uppermost groups however is due to a third factor, \emph{Distribution}: the uppermost group has level ``Bounded'' and the lowermost (or the middle group of the three) has level ``Sampled.''
...
<<factorialsigmadelta, pdfcrop=TRUE, echo=FALSE, warning=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Visualisation of the fitness changes at each time interval (top facet) and cumulative fitness change (bottom facet) for theta=0 with three values of delta (columns), and two values of sigma (each facet row.)'>>=
...
<<sampleenvironment, pdfcrop=TRUE, echo=FALSE, cache=TRUE, fig.pos='htp', fig.cap='Example time-series produced by our environmental model for a sample of theta, sigma and delta values. These are incremental fitness changes, rather than cumulative ones.'>>=
...
<<cumulativefitness, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, fig.pos='htp', fig.cap='Cumulative time-series examples for samples of theta, sigma and delta.'>>=
for (r in unique(t2$run)) {
t2[t2$run==r,'t'] <- 1:50 # tag with timestamp
...
As all three independent variables for our experiment, $\Theta$, $\sigma_e$ and $\delta$, are continuous in $\mathbb{R}$, and as we wish to test the specific relationship of fidelity across a range of these variables without being restricted to the initial choice of fixed levels, we shift from the fixed-effect factorial designs used earlier to a random effects model \parencite[chap.13]{Montgomery2009}. The values of the independent variables in each run are taken from a uniform random sample from their range, or in other words, a series of random samples with uniform probability from a space described as the hypercube with one variable on each of the three axes (see \cref{fig:hypercube}.)
...
We create a set of independent datasets by sampling from different four hypercubes, as defined in \cref{tbl:range-of-independent-variables}. The extremes of the ranges saw a lower proportion of the runs (representing more significant environmental changes or more rapid changes) reach completion than was seen in the centre (where the environment was stable.) The range of each variable (the length of a side of the hypercube) was adjusted from dataset to dataset to balance an adequate density of coverage across the overall range against the total run time required.
...
<<figure23, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, fig.pos='htp', fig.cap='Final mean fidelity against the environmental model\'s sigma parameter (top), delta parameter (middle) and theta parameter (bottom) for all experimental runs.'>>=
...
<<figure22, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, fig.pos='htp', fig.cap='Distribution of maximum generation reached for all runs for dataset no.1 (top) through dataset no.4 (bottom). Axes are to the same scale. Runs (along the x-axis) are ordered by final generation reached.'>>=
...
%<<figure25, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE,fig.pos='htp', fig.cap='Final mean fidelity against final mean fitness (top) and final mean fidelity against final standard deviation of fidelity (bottom).'>>=
...
A summary of the results from each of the four datasets is given in \cref{tbl:summary-for-changing-datasets}, and a visualisation of all four datasets combined given in \cref{fig:23}. Note that the difference in the proportion of completed runs to total runs reflects the respective ranges of the independent variables (as seen in \cref{tbl:range-of-independent-variables}), shown graphically in \cref{fig:figure22}.
...
\section{Is fidelity proportional to the predictability of the environment?}\label{mean-fidelity-predictability}
...
%<<figure24, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE,fig.pos='htp', fig.cap='Subset of results where $0.002 < \\lvert delta\\rvert$, again showing final mean fidelity (top) and final standard deviation of fidelity (bottom) against sigma parameter.'>>=
...
%@
%
%<<figure24a, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE,fig.pos='htp', fig.cap='Subset of results where $0.002 < \\lvert delta\\rvert$, again showing final mean fidelity (top) and final standard deviation of fidelity (bottom) against sigma parameter.'>>=
%results_low_bias <- subset(results, -0.002<delta & delta<0.002)
...
\section{Does the standard deviation of fidelity vary in proportion to the variability in the environment?}\label{sd-fidelity-predictability}
...
<<sd-fidelity, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, fig.pos='htp', fig.cap='Final standard deviation of fidelity against the environmental model\'s sigma parameter (top), delta parameter (middle) and theta parameter (bottom) for all experimental runs.'>>=
...
The corollary of this prediction is that the \gls{sd} of fidelity will be greater in unpredictable environments than in stable environments (when all three parameters are near zero). As a first approach, we note that in \cref{variance-of-inheritance-stable} we showed that in stable environments the variance of inheritance tended towards zero. As $\sigma_e$ is by definition unpredictable (see \cref{}) we can provide some support for the corollary if we can show that variance remains above zero for all $\sigma_e>0$ (incorporating all necessary uncertainties): 
...
\section{Is there a difference between changes applied by lineage and changes applied across an entire population?}
