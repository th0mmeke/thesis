...
We now introduce a general model of the relationship between variation and inheritance in a population of evolving abstract entities (\cref{base-model-algorithm}) where the key elements of the hypothesis are represented as explicit parameters. Bourrat summarizes this endogenous strategy as ``explaining variables which have previously been taken for granted in a model (such as reproduction and inheritance), by reference to other, more fundamental variables present in the model.'' \parencite[p.129]{Bourrat2015}.

The model consists of a population, consisting of abstract entities, and two simple functions--\emph{Selection} and \emph{Variation}--that each take a population as input and output a transformed population. Let's look at each element in turn.

\emph{Population} The population consists of $n$ entities, where $\lvert n\rvert\geq 0$.

Each \emph{entity} is fully described by two properties -- \emph{fitness} and \emph{fidelity}:
...
\emph{Selection} forms a new population by selecting elements from the current population, as defined in \cref{base-model-algorithm}. The probability of an element being included in the new population ($p_1$ in the algorithm) may be either a fixed value, or equal to its \emph{Fitness}. 

The new population created by \emph{Variation} consists of a number of new entities (``children'') for each entity (``parent'') in the current population. The properties of each child are related to the properties of its parent by a mapping, represented in the algorithm by the function $Derive$, which maps the parent value to a range given by a distribution of some range ($range$ in the algorithm) and an expected value equal to the parent's value (see \cref{fig:correlation}).

Each \gls{run} of the model consists of a fixed number of time steps (generations), where at each step these two functions are applied in sequence to the current population to form a replacement population.
...
\begin{algorithm}
	...
	\caption{Definitions for the functions Selection and Variation}\label{base-model-algorithm}
\end{algorithm}
