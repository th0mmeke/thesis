...
<<pdfcrop=TRUE, echo=FALSE, cache=TRUE>>=
df_full <- subset(load.results('results/results-819350e.data'),ecf==0 & truncate == 0 & correlation_correlation == 1) # only stable environment results
colnames(df_full)[13] <- "Distribution"
colnames(df_full)[14] <- "Reproduction"
...
@
<<figure24, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE,fig.pos='htp', fig.cap='Subset of results where $0.002\\lt \\lvert bias\\rvert$, again showing final mean fidelity (top) and final standard deviation of fidelity (bottom) against sd parameter.'>>=
library(ggplot2)
library(gridExtra)
library(cowplot) # styling of plots, extension of ggplot2
