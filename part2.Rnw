...
\begin{itemize}[label={}]
	...
	\item H$_1$:  $\sigma_{fidelity_{end}}$ is not close to $0$, for s0me clearly non-zero combinatins of $\Theta$,$\delta$ and $\sigma_e$.
\end{itemize}
