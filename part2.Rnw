...
df_full$n_{offspring} = factor(df_full$n_{offspring}, labels=c("2","5"))
df <- subset(df_full, gen==500)

df$distribution = factor(df$distribution, labels=c('Bounded normal','Sampled normal','Uniform'))
...
As might be expected, the higher reproduction rate in the right hand facet also resulted in a slightly higher proportion of runs reaching completion at \Sexpr{max(df_full['gen'])} generations: when $n_{offspring}=2$, \Sexpr{nrow(subset(df,n_offspring==2))} against \Sexpr{nrow(subset(df,n_offspring==5))} when $n_{offspring}=5$, 
...
ggplot(df) + geom_boxplot(aes(x=distribution,y=ave_cor)) + labs(x="", y="Final mean fidelity")
ggplot(df) + geom_boxplot(aes(x=distribution,y=ave_fit)) + labs(x="", y="Final mean fitness")
@
