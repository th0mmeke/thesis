...
# BYLINEAGE	CORRELATED	N_OFFSPRING	P_REPRODUCE	P_SELECTION	RESTRICTION	ar_sd ar_theta ave_cor	ave_fit	experiment	gen	pop	run	sd_cor	sd_fit
...
# ts <- replicate(300,arima.sim(list(order = c(1,1,0),  ar=runif(n=1,max=0.2), sd=0.1,n = 500))
# ts <- ts/max(abs(min(ts)),abs(max(ts))) * 0.3
# write.table(t(),'environments.csv',row.names=FALSE,col.names=FALSE, sep=',')
# plot.ts(ts[,1:10])
