...
\cite{Dittrich:2001zr} goes on to use these three components to classify a number of the \glspl{achem} of the time into six major types:
...
Another scheme was presented by Nellis with \glspl{ea} categorized as world/chemistry/constraints \parencite[p.132]{Nellis2012}, with constraints being energy model/binding model. As an example, Nellis describes RBN-World \cite{Faulconbridge2011} as a world where the elements are RBN networks.

\cite{Faulconbridge2011} takes a different approach, based on the relationship between a molecule's representation and its properties:
...
The reactor algorithm provides the mechanism to select (in effect, to order) reactions, or in the words of \cite[sect. 4.1.3]{Faulconbridge2011}, the ``...algorithm which describes the order of and intervals between reactions, starting from an initial collection of molecules...''
...
\item
Mixed scale: hierarchical spaces, such as aspatial cells within bigger grid, mostly for simulating biology (e.g., \parencite{Jeschke2008}). One possible advantage is the potential for parallelization.
\end{enumerate}

Scaling in the reactor algorithm can be an issue: as new products are generated, the space of reactions and reactants can expand exponentially, and therefore performance recuces in concert. The general solution is called model reduction--see \parencite{Radulescu2012} for a recent review; \parencite{Faulon2001} has a well-received method.

\chapter{ToyWorld}\label{toyworld}

ToyWorld, our \gls{achem} for the exploration of emergent behaviours, was first introduced in \cite{Young2013}. The elements of the model--Atoms, Molecules, Reactions, a Reaction Vessel--are recognisable from real-world chemistry, but in highly simplified forms.
...
\parencite{Faulconbridge2011} describes two generic strategies for the
...
\parencite{Benko2005,Benko2003}, RBN-World \parencite{Faulconbridge2011}
...
\chapter{Discussion}
\begin{NOTES}
Autocatalysis as a very simple form of inheritance and reproduction without variation. Pathway perhaps from this towards a form with variation, when hypothesis-2 kicks in...
\end{NOTES}
