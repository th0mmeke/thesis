...
``Synthesis and simulation of living systems'' or contemporary artificial life as \quote{an interdisciplinary study of life and life-like processes, whose two most important qualities are that it focuses on the essential rather than the contingent features of living systems and that it attempts to understand living systems by artificially synthesizing simple forms of them.}{\parencite{Bedau:2007ga}}
...
reactants and reactions (e.g., \parencite{Faulon2001}
\item
Realism fundamental
\item
...
as \parencite{Fontana1992}) to more chemically-realistic ones
\parencite{Suzuki2008a}.
\item
Examples
\item
\parencite{Hogerl2010} etc--Diels-Alder and Formose reaction networks
\end{itemize}
...
In practice the \textless{}\emph{S},\emph{R},\emph{A}\textgreater{} is more useful than this derived classification as the classification appears to be based on specific example chemistries rather than on sets of values for \textless{}\emph{S},\emph{R},\emph{A}\textgreater{}. Furthermore, some categories, such as Abstract Automata, are incompletely specified. Applying the scheme to a new, unclassified, chemistry that does not exactly match one of the given examples requires a somewhat subjective judgement as to the similarity between the molecule and reactions of the new chemistry and those of each example chemistry. Furthermore, because the system is incomplete, based as it is on given examples rather than on a partitioning of the some \textless{}\emph{S},\emph{R},\emph{A}\textgreater{} space, a new chemistry unrelated to any of the specific examples will either be classified in ``Other approaches'', or the scheme must be extended in an adhoc fashion.
...
\parencite{Tominaga2007} showed for a particular artificial chemistry, that it is computationally universal with only unimolecular and bimolecular reactions.
...
completely defined \parencite{Lenaerts2009}. Or in
\parencite[sect. 4.1.3]{Faulconbridge2011}, The mixing component of an
...
\parencite{Faulconbridge2011} identifies three types of \emph{mixing method} or reactor algorithm:
...
\begin{itemize}
	...
	grid, mostly for simulating biology (e.g., \parencite{Jeschke2008}). One possible
	advantage is potential for parallelization
\end{itemize}

Scaling can be an issue: as new products are generated, the space of reactions and reactants can expand exponentially, and therefore performance dives. The general problem is model reduction--see \parencite{Radulescu2012} for a recent review; \parencite{Faulon2001} has a well-received method.
...
\quote{Erasing the distinction in simulation between organism and environment allows a model to explore the exchange and transformation of matter and energy.} {\parencite{Dorin:2006fk}}
...
Catalysis/autocatalysis possible through emergence in some AChems (e.g., \parencite{Virgo2013})
...
\parencite{Benko2003,Benko2005} where reactions are modelled as a series
