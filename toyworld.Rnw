...
However, there is an earlier, and more influential, categorisation by \textcite{Dittrich:2001zr} which also coincidentally categorises \glspl{achem} against three factors, and which is commonly used in the literature (\eg \cite{Lenaerts2009,Gardiner2007}). This taxonomy is described in more detail in the \namecref{dittrich} below.
...
\begin{table}
	...
	\begin{tabular}{@{}p{7.5cm}p{6cm}@{}}
		...
		\cite{Fernando:2008xy,Fernando:2007pf}                             & No--atomic reactions                    \\
		\cite{Gardiner2007}                                                & No--atomic reactions                    \\
...
\Def{KineticReactantSelection(population)}{
	...
	\While{|ReactantList| = 0}{
		i $\leftarrow$ i + 1\;
		Advance the position of all molecules by $\Delta$t\ and update $ReactantList$ with all colliding molecules\;
