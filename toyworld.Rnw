...
In the taxonomy of \textcite{Dittrich:2001zr}, introduced in \cref{dittrich}, \glspl{achem} are classified along three axis: molecules, reaction rules, and the reactor algorithm. Our terminology, used in the remainder of this work, substitutes Reactant selection strategy ($S_\mathrm{Reactant}$) for the Reactor Algorthm of \textcite{Dittrich:2001zr}, and Product selection strategy ($S_\mathrm{Product}$) for Reaction rules. 
