...
Again, algorithm also applicable to F not just RAF - subset, simpler case.
...
\begin{algorithm}
	...
	\caption{Algorithm to identify all potentially autocatalytic cycles within a reaction network. A cycle is a sequence of reactions where at least one product in the final reaction is of the same species as a reactant in the first reaction of the cycle. The cycle is potentially autocatalytic if the stoichiometry of that product molecule is greater than one, or in other words, if the cycle produces more of the product than in consumes.}
\end{algorithm}
...
\begin{algorithm}
	...
	\For{CycleMolecules in MolecularCycles}{
	...
	\Return Clusters
	\caption{Cluster identification by grouping reaction cycles of the same form that share molecules.}
\end{algorithm}

\begin{algorithm}
	Autocatalytic$\leftarrow\emptyset$\;
    \For{CyclesForSpecies in MolecularCyclesBySpecies}{
		Clusters$\leftarrow$IdentifyCluster(CyclesForSpecies)\;
		\For{Cluster in Clusters}{
	...
	\caption{Autocatalytic set detection.}
\end{algorithm}


\section{Initial conditions}

The foodset contains 10 x [H][H] molecules, 10 x FO, 20 x O, 10 x [O-][N+](=O)[N+]([O-])=O, 10 x N(=O)[O], and 20 x O=C=O. Each replicate (run) of the experiment was for 
100,000 generations, with an initial ke for the reactor vessel of 50 units, switching every 10000 generations alternately between ke=140 and ke=50.
...
\begin{figure}
	\begin{center}
		...
		\caption{Conceptual model for experiments incorporating environmental change. As described in \cref{environmental-models}, the \emph{Target} experimental factor affects the Evolutionary model; the \emph{Shape} factor modifies the Environmental model.}
...
df_1 <- read.csv('results/stablestates.csv')
names(df_1)[9]<-"dataset"
df_1[,"dfa"] <- NULL # use correct dfa value from summary-metrics
df_2 <- read.csv('results/summary-metrics.csv')
df <- merge(df_1,df_2, by=c('experiment','environment','dataset'), all.x=TRUE)
...
@
<<>>=
ggplot(df) + geom_point(aes(x=H, y=mean))
ggplot(df) + geom_point(aes(x=df$"Sample Entropy", y=mean))
ggplot(df) + geom_point(aes(x=seeds, y=mean))
@
