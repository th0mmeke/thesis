...
\subsubsection{Available energy function}
...
Long-chain construction by Polymerization

Polymers formed by chains of monomers (molecules with particular structure allowing chains to form.) Can Toyworld2 produce polymers? Preferential product selection mechanism might do so, over least energy method. For example, C=CCl (Chloroethylene) doesn't naturally form chains (PVC) under least-energy; neither does C=C.

Polymerization is not possible with a single-step reaction mechanism where the molecules are legal after the first step: it requires changing a double bond to a single bond (\eg C=C to CC) in each monomer and the simultaneous formation of a single CC bond between the two monomers.

The initial population and foodset contained 10x[H][H], 10xFO, 20xO, 10x[O-][N+](=O)[N+]([O-])=O, 10xN(=O)[O], and 20xO=C=O.
...
ap <- ggplot(df) + geom_point(aes(x=dfa, y=mean)) + labs(x='Hurst exponent', y='Mean number of stable states')
bp <- ggplot(df) + geom_boxplot(aes(x=dfa>0, y=mean, group=dfa>0)) + labs(x='Environment state', y='Mean number of stable states') + scale_x_discrete(labels=c("Stable","Varying"))
