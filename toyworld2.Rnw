...
\begin{algorithm}
	...
	\While{|ReactantList| = 0}{
		...
		AddList$\leftarrow$chhose TURNOVER new molecules from foodset with uniform probability, and give each the same ke as the initial set of molecules\;
	...
	\Return $\emptyset$
	\caption{Flow selection strategy}\label{alg:flow-selection}
\end{algorithm}
...
100,000 generations, with an initial ke for the reactor vessel of 50 units, switching every 10000 generations alternately between ke=140 and ke=50. At every step, 5 molecules randomly chosen molecules were removed from the reaction vessel and replaced by 5 new molecules chosen with uniform probability from the foodset (TURNOVER=5.) 
