...
\end{tikzpicture}
\caption{Example multiplier from experimental results (of type 'Y' - see \cref{tbl:replicator-species}). For clarity, the only molecules shown are those that form part of the multiplier cycle (all others can be easily discovered from the reaction equations.)}
