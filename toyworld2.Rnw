...
\begin{algorithm}
	\Def{IdentifyCycles(Reactions, MaxCycleSize)}{
		...
		\For{each $Seed$ in $Reactants$}{
			\For{each $Candidate$ in list of all molecules in the reaction graph of the same molecular species}{
				\For{each $Path$ in all shortest paths of length < $MaxCycleSize$ from $Candidate$ to $Seed$}{
	...
	\caption[\emph{IdentifyCycles}. Algorithm to identify all potentially autocatalytic cycles within a reaction network.]{\emph{IdentifyCycles}. Algorithm to identify all potentially autocatalytic cycles within a reaction network, using the definition of autocatalysis from \cref{context}. A cycle is a sequence of reactions where at least one product in the final reaction is of the same species as a reactant in the first reaction of the cycle. The cycle is potentially autocatalytic if the stoichiometry of that product molecule is greater than one, or in other words, if the cycle produces more of the product than is consumed. $MaxCycleSize$ is used to bound the cost of the shortest path search.}\label{alg:identify-cycles}
\end{algorithm}
...
\begin{algorithm}
	...
	\caption[\emph{IdentifyClusters}. Identification of clusters of reaction cycles of the same form that also have one or more molecules in common.]{\emph{IdentifyClusters}. Identification of clusters of reaction cycles of the same form that also have one or more molecules in common. \emph{MolecularCyclesForSpecies} contains all cycles of the same cycle species, that is, all cycles that share the same sequence of reactions when converted to SMILES (species) form.}\label{alg:identify-clusters}
\end{algorithm}

\begin{algorithm}
	\Def{IdentifyMultipliers(MolecularCyclesBySpecies)}{
	...
	\caption[\emph{IdentifyMultipliers}. Detect multiplying replicators within a reaction network.]{\emph{IdentifyMultipliers}.Detect multiplying replicators within a reaction network. The input \emph{MolecularCyclesBySpecies} is the output of \emph{IdentifyCycles} grouped by cycle species, that is, grouped into sets of cycles that share the same sequence of reactions when converted to species form.}\label{alg:identify-autocatalytic-cycles}
\end{algorithm}
...
%	return stable_cycles, [x[0] for x in cycle_form.itervalues()]
	...
	\caption{\emph{IdentifyMultipliers}. Algorithm to identify all multipliers within a reaction network.}\label{alg:identify-multipliers}
	
\end{algorithm}

\section{Evolutionary model}

%The evolutionary model used is the same as previously described, with the exception of the reactant selection strategy, $S_{reactant}=LocalReactantSelection$. The FlowReactantSelection algorithm (\cref{alg:flow-selection}) is a refinement of the earlier UniformReactionSelection strategy, modified to provide support for a FoodSet and spatial reactant selection through locality.
...
Other parameters were set as follows: for dataset 1487376025 only $MaxCycleSize=7$ (only cycles containing seven or fewer molecules are included); for other datasets $MaxCycleSize=10$.
...
#r <- csv_replicators[csv_replicators$Dataset=='1484540618',c("Dataset","Replicate","Clusters","Replicators")]
r <- csv_replicators[csv_replicators$Environment==0,c("Dataset","Replicate","Clusters","Replicators")]

library(plyr)
library(xtable)
print(xtable(r, caption='Summary results from applying \\cref{ch:multiplication} to datasets generated by various $S_{reactant}$, all though under stable environments (no external selective pressure).', label='tbl:toyworld2-replicator-results'), booktabs=TRUE, include.rownames=FALSE, size="scriptsize", caption.placement="top")
@
...
\subsection{Reactant selection strategy}
...
\subsection{Product selection strategies}
...
The experiments follow a modified full factorial-design over two factors (shown in \cref{ch:variability}): factor \emph{Target} has two levels, and factor \emph{Shape} with three levels. Environmental variability is approximated by the sample entropy of the sequence of changes generated by an environmental model determined the value of the \emph{Shape} factor.
