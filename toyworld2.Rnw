...
From the earlier definition of replication (\cref{non-informational-exact-replicators}), given by the autocatalytic reaction $\Sigma x_i + A\rightarrow \Sigma y_j + \Sigma B_k$, $A$ is a multiplying entity if $A$ and some $n<k$ of the $B_k$ are equivalent under selection \parencite{Zachar2010}. As a secondary goal, we are also interested to learn if our underlying hypothesis from \cref{toyworld}, that simple measures of cycle formation in a chemistry can predict the subsequent emergence of replicators, holds at this next level of investigation.
...
\begin{itemize}
	...
	\item The reaction cycle species has \textit{stoichiometry greater than one}stoichiometry greater than one, and
...
Our model design is informed by the results of \cref{toyworld}. We use the ToyWorld parameterized evolutionary model with a single experimental factor, $S_{product}$, with two levels of LeastEnergy and Uniform (\cref{product-selection-strategies}.), while $S_{reactant}$ is set to KineticReactantSelection (\cref{alg:kinetic-selection}).

There are two replicates for each of the two factor levels, and each replicate runs for 100,000 generations. The initial population for each replicate contains 10$\times$[H][H] molecules, 10$\times$FO, 20$\times$O, 10$\times$[O-][N+](=O)[N+]([O-])=O, 10$\times$N(=O)[O], and 20$\times$O=C=O, and the initial reactor kinetic energy is set to 100 units, again following follow the parameter values established in \cref{toyworld}.
...
\subsection{Do multipliers arise simply by chance?}
...
In summary, exact multipliers do arise in the ToyWorld \gls{achem}, but not in any great numbers, and when they do, they do not persist for long. We have shown though that they do occur as the result of a non-neutral combination of Product and Reactant selection strategies, and not purely by chance.
...
Although the shape can clearly take an infinite number of different forms, we concentrate on two representative forms, one adopted from \citeauthor{Jablonka1995,Paenke:2007ie} and one novel:
...
Although selective pressure can be purely internal to the reactor, for example in the competition between reaction cycles for molecules \parencite{Eigen1971}, this is inherently more difficult to model and to control than external selective pressure applied explicity. In these experiments we concentrate solely on external selective pressure.
...
\begin{itemize}
	\item \emph{Bistate selective pressure}. Alternate every 10,000 generations between -20 and +20 values.
	\item \emph{AR-timeseries}, following \cref{environmental-model}, described by \cref{ar-1-time-series} with three parameters $\Theta$, $\sigma_e$ and $\delta$. $\Theta$ is the AR coefficient, $e_t$ is a random, normally distributed, error component around a mean of $0$, where $e_t\stackrel{iid}{\sim}N(0,\sigma^{2}_e)$, and $\delta$ is a fixed bias value. The timeseries values are applied to the target of change every 1000 generations.
\end{itemize}

Target of change:

\begin{itemize}
	\item \emph{Kinetic Energy}.
	\item \emph{Reactor population}.
\end{itemize}

Measure of AR-timeseries variability:
