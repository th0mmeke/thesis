...
The evolutionary model used is the same as previously described, with the exception of the reactant selection strategy, $S_{reactant}=FlowReactantSelection$. The FlowReactantSelection algorithm (\cref{alg:flow-selection}) is a refinement of the earlier KineticReactionSelection strategy, modified to provide support for a FoodSet.
...
f\section{Results and discussion}
