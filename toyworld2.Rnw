...
colClasses <- c("factor","integer","integer","integer","factor","factor","factor","factor","integer")
...
drop <- c('S_Reactant', 'Target', 'Shape')

library(xtable)
print(xtable(csv_multipliers[csv_multipliers$Environment==0,!names(csv_multipliers) %in% drop], caption='Summary results for numbers of multipliers measured by \\cref{alg:discover-multipliers} in reaction networks generated by LeastEnergy and Uniform $S_{product}$ strategies, under stable environments (no external selective pressure).', label='tbl:toyworld2-multiplier-results'), booktabs=TRUE, include.rownames=FALSE, size="scriptsize", caption.placement="top")
@
