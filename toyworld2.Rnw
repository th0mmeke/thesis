...
\begin{algorithm}
	...
	\caption[\emph{FindCyclesFromSeed}. Recusive algorithm to identify cycles from a seed molecule.]{\emph{FindCyclesFromSeed}. Recursive breadth-first algorithm to identify all shortest paths (and hence cycles) from $Source$ to any predecessor molecule of the same molecular species as $Source$ in the directed reaction graph.}\label{alg:find-cycles-from-seed}
\end{algorithm}
