...
An environmental model must describe three particular components of environmental change: the \emph{scope} of the change, the \emph{target} of the change, and the \emph{shape} of the change. First, we discuss scope; how each change is experienced by the entities in the population. We can consider three different levels of scope, from most homogeneous to least:
\begin{enumerate}
...
We test two targets of environmental variability: changes to the size of the foodset, and changes to the available energy for reactions.
...
\begin{table}
	...
	\begin{center}
		...
		\begin{tabular}{p{1.4cm}p{2.2cm}p{4.4cm}p{5.5cm}}
			...
			\midrule
			$Target$ 		& Kinetic							& Uniform		& \\
			$Shape$  		& LeastEnergy                       & Uniform		& \\
			\bottomrule
		\end{tabular}
	\end{center}
\end{table}
...
The primary metric in the hypothesis is \emph{stable cycle states}. That is, a chain of two or more cycles linked by a product in one cycle being a reactant in the following cycle, and where the cycles have the same "form", or sequence of molecule species.
...
<<anova, results='asis', echo=FALSE, cache=TRUE, warning=FALSE>>=
