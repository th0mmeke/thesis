...
<<toyworld2-results-by-target, pdfcrop=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, fig.pos='ht', fig.scap=NA, fig.cap='Mean number of stable cycle forms (left) and instances of autocatalytic cycles (right) for each environment against variation in the environment measured by sample entropy.'>>=
