...
\chapter{The Emergence of Multiplication}\label{ch:multiplication}
...
\section{Approach}\label{emergent-measure}
...
\begin{algorithm}
	...
	\caption[\emph{IdentifyCycles}. Algorithm to identify all potentially autocatalytic cycles within a reaction network.]{\emph{IdentifyCycles}. Algorithm to identify all potentially autocatalytic cycles within a reaction network, using the definition of autocatalysis from \cref{context}. A cycle is a sequence of reactions where at least one product in the final reaction is of the same species as a reactant in the first reaction of the cycle. The cycle is potentially autocatalytic if the stoichiometry of that product molecule is greater than one, or in other words, if the cycle produces more of the product than is consumed. $MaxCycleSize$ is used to bound the cost of the shortest path search.}\alg{identify-cycles}
\end{algorithm}
