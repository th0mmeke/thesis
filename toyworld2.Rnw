...
names(df)[11] <- "State"
with(df, {
	av <- anova(lm(mean~Target*State*Hurst))
...
ap <- ggplot(df) + geom_point(aes(x=Hurst, y=mean, color=State)) + labs(x='Hurst exponent', y='Mean number of stable states', color='Environment') + scale_colour_manual(name="",  values=c("grey80", "grey50", "black"))
