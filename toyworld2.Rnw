...
%csv <- load.multipliers('results/multipliers.csv')
%
%drop <- c('S_Reactant', 'Target', 'Shape', 'Sample Entropy', 'DFA')
%
%library(xtable)
...
csv1 <- load.multipliers('results/multipliers.csv')
...
All runs of the experiment resulted in multipliers by our earlier definition. The numbers are not large, but they are non-zero and so sufficient to reject H$_0$ in favour of the alternate hypothesis that multipliers can emerge in a \gls{achem}.
...
No replicate of the control experiments generated multipliers, and so we reject this alternate explanation (that multipliers result from chance alone) in favour of our modified hypothesis, that multipliers emerge from biases in the \gls{achem}. Although the sampling proportion $p$ undoubtably affected the total number of multipliers identified in each experiment, the relative numbers between experiments are maintained and hence we do not believe that this result is an artifact of sampling.
...
<<toyworld2-vmultiplier-020-results, fig.pos='ht', results='asis', echo=FALSE, cache=TRUE, warning=FALSE>>=
	colClasses <- c("factor","integer","integer","integer","factor","factor","factor","factor","numeric","numeric","integer","integer","numeric")
	csv <- read.csv('results/multipliers-020.csv', colClasses=colClasses)
	names(csv)[names(csv)=="Datetime"] <- "Dataset"
	names(csv)[names(csv)=="Sample.Entropy"] <- "Sample Entropy"
	names(csv)[names(csv)=="Multiplier.Species"] <- "No. of Species"
	names(csv)[names(csv)=="Average.Lineage.Size"] <- "Average Lineage Size"
